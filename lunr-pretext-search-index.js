...
{
  "id": "sec-population-discrete-25",
  "level": "2",
  "url": "sec-population-discrete.html#sec-population-discrete-25",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "loop iterative "
},
{
  ...
  "body": " Density Dependent Population Growth Models  A population with constant per capita growth rates sees exponential growth when the birth rate exceeds the death rate , and exponential decay when the ordering is reversed. However, real population can not really grow exponentially as there iare limited resources in the environment. Thus, for populations that experience environmental constraints, constant per capita growth rates are not appropriate.  We previously defined the per capita net growth rate where is the per capita birth rate and is the per capita death rate. Sometimes it is inconvenient or impossible to determine the birth and death rates separately. If we can at least measure the net change in the population, which will be equal to , then the per capita net growth rate will be a sequence defined by Density dependence means that the relation is not constant, but that depends on .  We turn our attention to the case that the per capita net growth rate can be modeled as a function of the population size , where a scatter plot of the points could be reasonably approximated by a function. We generally expect that the graph will be a decreasing function because as the population gets larger, the growth rate should decrease. For a very large population, we expect since the population would be greater than its carrying capacity. Physically, however, we will always have , with corresponding to complete extinction.   If we revisited the density-dependent agent-based model in Section , we can plot the per capita net growth rate computed at each time step with respect to the size of the population at the beginning of the step. The figure below illustrates this relationship. Values when the population was small and demographic stochasticity is high were not included.     We see in the graph that the per capita net growth rate is decreasing. The data seem consistent with a model where the per capita rate decreases linearly. This makes sense because the rules of the agent-based model were that the death rate increased linearly in terms of the number of individuals on a shared patch.   In order to develop an actual model, we need to find a formula that captures the relation. There are infinitely many different formulas that we might try to use, especially if we only have a few data points. To be more precise, we generally choose families of formulas that are simple and are described by only a few parameters . A parameter is a symbol that represents a constant that appears in the formula, like a variable that helps describe the model while not necessarily representing anything physical in the actual state of the system.  The simplest density dependent model to write down would be when is a linear function of , The values and are the parameters of the model. Here is called the intrinsic per capita net growth rate and represents the per capita net growth rate when . For a small population, far from the carrying capacity, this represents the value of that looks like an exponential growth model until the population size starts to cause density dependent effects. The parameter is the size of the slope indicating how much the growth rate decreases per individual.   Using the density dependent data from our agent-based model, we can use linear regression to find a line of best fit. This gives us the following values for the parameters: The resulting parametric model for the per capita growth rate is then given by A graph showing the trend line with the data is given below.      "
},
{
  "id": "sec-population-density-dependent-5",
  "level": "2",
  "url": "sec-population-density-dependent.html#sec-population-density-dependent-5",
  "type": "Example",
  "number": "2.3.1",
  "title": "",
...
{
  "id": "sec-population-density-dependent-6",
  "level": "2",
  "url": "sec-population-density-dependent.html#sec-population-density-dependent-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "sec-population-density-dependent-7",
  "level": "2",
  "url": "sec-population-density-dependent.html#sec-population-density-dependent-7",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "parameters intrinsic per capita net growth rate "
},
{
  "id": "sec-population-density-dependent-8",
  "level": "2",
  "url": "sec-population-density-dependent.html#sec-population-density-dependent-8",
  "type": "Example",
  "number": "2.3.3",
  "title": "",
