...
{
  "id": "subsec-diffeq-numerical-solutions-6",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-numerical-solutions-6",
  "type": "Example",
  "number": "4.1.4",
  "title": "",
  "body": "  Find a numerical approximation for the solution of the differential equation with the intial value and compare it to the exact solution we found earlier .    Our R script will need to first call library(deSolve) . Then we will define our rate function to represent the formula . This function is required to have three input arguments time, the state, and parameters for the model. The state will eventually include all of our state variables, but for now will only consist of the single variable . Our model does not have any additional parameters, so the parameters will not actually be used in this example. Once the differential equation is defined, we provide the initial value and times we wish to evaluate.   R script to generate numerical solution to differential equation   # Load the library library(deSolve) # Define the rate function in this form rateFcn <- function(t, state, params) { with(as.list(c(state, params)), { # This lets us use a simple formula dQ <- 3*Q list(dQ) # Return the rate }) } # Define the initial condition init_state <- c(Q = 5) params <- c() # empty # Create an array of times where we evaluate times <- seq(from = 0, to = 2, by = 0.1) # Find the solution # Will have column for time and each state variable solution <- ode(init_state, times, rateFcn, params) # Make a graph to compare library(ggplot2) soln_graph <- ggplot() + geom_point(data = solution, mapping = aes(x = time, y = Q, color = \"1\")) + geom_function(fun = function(t) { 5*exp(3*t) }, mapping = aes(color = \"2\")) + scale_color_discrete(name = \"solution\", breaks = c(\"1\", \"2\"), labels = c(\"numerical\", \"exact\") ) show(soln_graph)     "
},
{
