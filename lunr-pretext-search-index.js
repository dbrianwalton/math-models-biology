...
},
{
  ...
  "number": "1.5.4",
  "title": "",
  "body": " ggplot(US_population_data, mapping = aes(x = Year, y = Population) ) + geom_point(color = \"blue\") + coord_cartesian(ylim = c(0, 4e8))  "
},
{
  ...
  "number": "1.5.5",
  "title": "",
  "body": " library(dplyr) focus_population_data <- filter(US_population_data, Year >= 1991 & Year < 2010) ggplot(focus_population_data, mapping = aes(x = Year, y = Population) ) + geom_point(color = \"blue\") + coord_cartesian(ylim = c(0, 3.5e8))  "
},
{
  ...
  "number": "1.5.6",
  "title": "",
  "body": " ggplot(focus_population_data, mapping = aes(x = Year, y = Population) ) + geom_point(color = \"blue\") + geom_point(aes(y = \"Births\"), color = \"red\") + geom_point(aes(y = \"Deaths\"), color = \"green\") + coord_cartesian(ylim = c(0, 3.5e8))  "
},
{
...
{
  "id": "linear-projection-analysis-6",
  "level": "2",
  "url": "projection-analysis.html#linear-projection-analysis-6",
  "type": "Example",
  "number": "2.5.3",
  "title": "",
...
{
  "id": "linear-projection-analysis-8",
  "level": "2",
  "url": "projection-analysis.html#linear-projection-analysis-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "linear-projection-analysis-11",
  "level": "2",
  "url": "projection-analysis.html#linear-projection-analysis-11",
  "type": "Example",
  "number": "2.5.5",
  "title": "",
...
},
{
  ...
  "number": "2.5.7",
  "title": "",
  "body": "  If a function is differentiable at (that is, exists), then we can define the linear approximation of around . The linearization is the linear function with slope and passing through the point given by . When is sufficiently close to , the functions are nearly indistinguishable.    The tangent line equation is a consequence of standard calculus definitions. To clarify the idea of sufficiently close without trying to recreate an entire course in analysis, let us start by pointing out that because exists, the average rate of change defined by must converge to as approaches , as given in the limit . This is equivalent to saying . This can be rewritten as . Because the denominator goes to zero, the numerator must go to zero even faster for the limit to equal zero.   "
},
{
...
{
  "id": "ex-discrete-logistic-projection-analysis",
  "level": "2",
  "url": "projection-analysis.html#ex-discrete-logistic-projection-analysis",
  "type": "Example",
  "number": "2.5.9",
  "title": "",
  "body": "  Use computer analysis to find the fixed points for the discrete logistic model . Find the slopes for each fixed point and then discuss how the stability depends on the values of the parameters.    The projection function is , which is a function of involving two additional parameters and . To perform our analysis using Sage, we will need to declare all three symbols as mathematical variables. Nevertheless, we can still solve the fixed point equation for , and Sage will provide answers in terms of the paramters.   When you evaluate that work, we will match results we would get using basic calculus. If , we know how to find a derivative as . Similarly, we can algebraically write down the fixed point equation and solve it by hand: Because this equation says the product of with equals zero, the solutions are found by setting each factor individually to zero, giving us and as our two solutions. This matches what Sage tells us.  From our Sage work, or by evaluating , we find . This means that whenever , . By , we now know that is an unstable fixed point whenever It is actually possible to have if the per capita death rate exceeds the per capita birth rate. In that circumstance, we have . Consequently, is a stable fixed when the death rate exceeds the birth rate for .  For the second fixed point , we have . We think through the same conditions as above. If (negative ), we have but we also have a fixed point with a negative value. In the current setting for populations, does not make sense. But mathematically, such a negative fixed point would be unstable. If , we have between 0 and 1, which means the fixed point is locally stable and sequences converge without oscillation. Once we have , we have so that nearby sequences oscillate above and then below the fixed point. For , we have and the oscillation decays away to the stable fixed point. Once we have , this causes such that oscillation grows and the fixed point is locally unstable.   "
},
{
...
},
{
  ...
  "number": "",
  "title": "",
  "body": "discrete logistic "
},
{
  "id": "projection-compare-replacement-7",
  "level": "2",
  "url": "projection-cobweb.html#projection-compare-replacement-7",
  "type": "Figure",
  "number": "2.6.1",
  "title": "",
  "body": "    "
...
{
  "id": "projection-compare-replacement-8",
  "level": "2",
  "url": "projection-cobweb.html#projection-compare-replacement-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "carrying capacity "
...
{
  "id": "projection-compare-replacement-10",
  "level": "2",
  "url": "projection-cobweb.html#projection-compare-replacement-10",
  "type": "Figure",
  "number": "2.6.2",
  "title": "",
  "body": "    "
...
{
  "id": "cobweb-diagrams-5",
  "level": "2",
  "url": "projection-cobweb.html#cobweb-diagrams-5",
  "type": "Listing",
  "number": "2.6.7",
  "title": "",
  "body": " # Define the projection function my_proj <- function(x, params) { with(params, { # Expecting values for r0 and K x_out <- x + r0*x*(1-x\/K) } ) } # Parameters for this example my_params <- list(r0 = 0.75, K = 10) # Initial value and number of steps x0 <- 1 num_steps <- 10 # Window for the plot x_min <- 0 x_max <- 15 # Generate the smooth functions xvals <- seq(from = x_min, to = x_max, length.out = 100) yvals <- my_proj(xvals, my_params) library(ggplot2) base_plot <- ggplot() + geom_line(mapping = aes(x = xvals, y = yvals), color=\"blue\", linewidth=1) + geom_abline(intercept = 0, slope = 1) + coord_cartesian(xlim = c(x_min, x_max), ylim = c(x_min, x_max)) + labs(x = \"x[n]\", y = \"x[n+1]\") # Generate the sequence used for the cobweb x_seq <- double(num_steps + 1) x_seq[1] <- x0 for (n in 1:num_steps) { x_seq[n+1] <- my_proj(x_seq[n], my_params) } # Create a sequence of points. cobweb_x <- double(2*num_steps + 1) cobweb_y <- double(2*num_steps + 1) # First point at (x0, 0) cobweb_x[1] <- x0 cobweb_y[1] <- 0 # For each step, add two points for (n in 1:num_steps) { # Move vertically to projection function cobweb_x[2*n] <- x_seq[n] cobweb_y[2*n] <- x_seq[n+1] # Move horizontally cobweb_x[2*n+1] <- x_seq[n+1] cobweb_y[2*n+1] <- x_seq[n+1] } # Draw the cobweb on top of the base plot cobweb_plot <- base_plot + geom_path(mapping = aes(x = cobweb_x, y = cobweb_y), color=\"orange\", linewidth=1) cobweb_plot  "
},
{
...
},
{
  ...
  "number": "2.7.1",
  "title": "",
  "body": " Consider the discrete logistic model using the projection function parametrized as . As discussed in , the natural fixed points for this model are at and at . Once constant harvesting is introduced, the modified projection function is given by .  The fixed points are the solution to the quadratic equation . The solutions are given by the quadratic formula . When , we recover the unperturbed fixed points. As increases, the expression inside the square root, , decreases until it equals zero at the value . For any value of , there are no fixed points, a catastrophic result for the population that is predicted to decrease indefinitely.  You should note that for , is no longer a fixed point. Mathematically, even though there are no individuals, the model still harvests individuals resulting in a negative population. Although this does not make physical sense, it is biologically significant for very small populations. It would indicate that for small enough populations, constant harvesting would eradicate the population in a single step. And for populations that are below the lower fixed point, the population would be decreasing steadily towards this same fate.  You can explore the cobweb diagram for this situation for the discrete logistic model with and and varying values for . The bifurcation occurs when . Notice that if your initial condition is below the lower fixed point, the population will die off. Also notice that if is above the bifurcation value, there is no population that will be able to survive.     "
},
{
...
{
  "id": "population-constant-harvesting-5",
  "level": "2",
  "url": "population-interventions.html#population-constant-harvesting-5",
  "type": "Listing",
  "number": "2.7.3",
  "title": "",
  "body": "  # Parameters for the diagram r0 <- 1.8 a <- 0.01 # We calculated the bifurcation point as H=81 H_vals <- seq(from = 0, to = 81, length.out = 100) # For these values of H, calculate the two fixed point locations fp1 <- (r0 + sqrt(r0^2 - 4 * a * H_vals))\/(2*a) fp2 <- (r0 - sqrt(r0^2 - 4 * a * H_vals))\/(2*a) # Make a graph library(ggplot2) ggplot() + labs(x = \"H\", y = \"P\") + geom_line(mapping = aes(x = H_vals, y = fp1, color=\"stable\"), linewidth = 1) + geom_line(mapping = aes(x = H_vals, y = fp2, color=\"unstable\"), linewidth = 1, linetype = \"dotted\") + scale_color_manual(name = \"Stability\", values = c(\"stable\" = \"blue\", \"unstable\" = \"red\"))   "
},
{
  "id": "population-constant-harvesting-8",
  "level": "2",
  "url": "population-interventions.html#population-constant-harvesting-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  ...
  "number": "2.7.4",
  "title": "",
  "body": " Again consider the discrete logistic model parameterized by the projection function with fixed points at and at . With proportional harvesting, the modified projection function is given by   The fixed point equation can be rewritten in an equivalent factored form . The fixed points of the perturbed model are and . When , the second fixed point corresponds to our original value . As increases, the location of the second fixed point steadily decreases. When , the two fixed points overlap, and when , the second fixed point is a negative value.  You can explore the cobweb diagram for this situation for the discrete logistic model with and and varying values for . The bifurcation occurs when . Notice that if your initial condition is between the two fixed points, the population increase. But if the harvesting rate exceeds the intrinsic per capita net growth rate , then the population converges to an extinct state.     "
},
{
  "id": "population-interventions-4-6",
  "level": "2",
  "url": "population-interventions.html#population-interventions-4-6",
  "type": "Listing",
  "number": "2.7.6",
  "title": "",
  "body": "  # Parameters for the diagram r0 <- 1.8 a <- 0.01 # We calculated the bifurcation point as h=1.8 h_vals_1 <- seq(from = 0, to = 1.8, length.out = 10) fp1_1 <- rep(0, length(h_vals_1)) # Need as many \"0\" as values of h fp2_1 <- (r0 - h_vals_1)\/(a) h_vals_2 <- seq(from = 1.8, to = 2.5, length.out = 10) fp1_2 <- rep(0, length(h_vals_2)) # Need as many \"0\" as values of h fp2_2 <- (r0 - h_vals_2)\/(a) # Make a graph library(ggplot2) ggplot() + labs(x = \"h\", y = \"P\") + geom_line(mapping = aes(x = h_vals_1, y = fp1_1, color=\"unstable\"), linewidth = 1, linetype = \"dotted\") + geom_line(mapping = aes(x = h_vals_2, y = fp1_2, color=\"stable\"), linewidth = 1) + geom_line(mapping = aes(x = h_vals_1, y = fp2_1, color=\"stable\"), linewidth = 1) + geom_line(mapping = aes(x = h_vals_2, y = fp2_2, color=\"unstable\"), linewidth = 1, linetype = \"dotted\") + scale_color_manual(name = \"Stability\", values = c(\"stable\" = \"blue\", \"unstable\" = \"red\"))   "
},
{
  "id": "population-interventions-4-9",
  "level": "2",
  "url": "population-interventions.html#population-interventions-4-9",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  ...
  "number": "3.1.4",
  "title": "",
  "body": "  A pair of sequences with a linear projection model will asymptotically behave like geometric sequencees. That is, the ratios of values and both converge to a constant called the asymptotic geometric growth rate. Furthermore, suppose we calculate the total and look at the ratios and , which represent the fractions of the total count in seed and plant form. The ratios always converge to values and , called the stable stage distribution , that are independent of initial values.   "
},
{
...
{
  "id": "sec-structure-matrix-models",
  "level": "1",
  "url": "sec-structure-matrix-models.html",
  "type": "Section",
  "number": "3.2",
  "title": "Matrix Models for Structured Populations",
  "body": " Matrix Models for Structured Populations  Our two-stage model had a very specific structure that we need to explore. The projection formulas were written as a sum of terms proportional to our state variables. Such a formula is called a linear combination .    Given any mathematical objects that can be multiplied by constants and added together, say , and a corresponding number of constants, , the sum is called a linear combination of .    When we have state variables , a recursive model requires that we define projection formulas, one for each variable. If every projection formula is a linear combination of the state variables, then the entire system is specified by knowing the constants. The constants have two subscripts, where the first subscript refers to which row or equation the constant appears in and the second subscript refers to which variable is being multiplied. So would appear in the 3rd equation\/row and be multiplied by the 5th variable. Knowing these coefficients is equivalent to knowing the full system of equations.   If a particular state variable does not appear in the linear combination, its corresponding coefficient should be 0.   A matrix is a rectangular array of numbers. We can represent the system of projection equations with a matrix called a projection matrix , . If we represent the state values in vector form , then the system of projection formulas can be summarized more concisely with a matrix-vector equation, . The mathematical study of these objects and the formal meaning of matrix-vector equations are taught in a subject called linear algebra .    Consider a population of annual plants with a seedbank such that a seed in the seedbank has a probability 0.6 of germinating each year and a probability 0.2 of surviving in the seedbank to the next season, and where half of plants survive the growing season and subsequently produce an average of 10 seeds. Following the model development in , our projection model can be written . Our projection matrix becomes . If we define our state vector as , then the model can be written .  In R, we can create a matrix using the matrix command. This command expects data which will fill columns by default, and a specification of the number of rows and columns. The matrix above would be computed in R as matrix(c(0.08, 0.12, 2, 3), nrow=2, ncol=2) . Matrix-vector multiplication is performed using %*% . The following script illustrates how we would calculate sequence values with initial values .    # Define the projection matrix. M <- matrix(c(0.08, 0.12, 2, 3), nrow=2, ncol=2) # Initial values S0 <- 20 P0 <- 0 # Create space for the state vectors to be stored (as a matrix with 2 columns) N <- 30 X <- matrix(0, nrow=N+1, ncol=2) # Put initial value in 1st row # To refer to a row, leave column blank X[1, ] <- c(S0, P0) for (n in 1:N) { # Fill n+1 row of state values with project from n row of state values X[n+1, ] <- M %*% X[n, ] } library(tidyr) library(dplyr) # Generate a data-frame, calculate totals and stage distribution # S is in column 1 of X, P is in column 2 seedbank_data <- tibble(t = 0:N, S = X[ ,1], P = X[ ,2]) |> mutate(total = S + P) |> mutate(rho_S = S\/total, rho_P = P\/total) # Calculate growth ratio seedbank_data$lambda_S <- NA seedbank_data$lambda_P <- NA seedbank_data$lambda_S[2:(N+1)] <- seedbank_data$S[2:(N+1)] \/ seedbank_data$S[1:N] seedbank_data$lambda_P[2:(N+1)] <- seedbank_data$P[2:(N+1)] \/ seedbank_data$P[1:N]    After running this code, we discover that this population has an asymptotic growth ratio and a stable stage distribution with and .    Every matrix that has the same number of rows as columns is called a square matrix . From linear algebra, we learn that every square matrix has a collection of characteristic values or eigenvalues , up the number of rows\/columns. Associated with each eigenvalue is one or more vector, called an eigenvector  . The key feature of an eigenvector\/eigenvalue pair is that the matrix-vector multiplication just rescales by the eigenvalue, . Repeated multiplication by therefore results in exponential growth or decay in the direction of that vector, . The asymptotic growth ratio corresponds to the largest eigenvalue of . Furthermore, the stable stage distribution will be an associated eigenvector to that eigenvalue.    Let be a square matrix. The dominant eigenvalue of is the eigenvalue whose absolute value is the largest of all eigenvalues. Under typical conditions for population projection matrices (irreducible, aperiodic, and non-negative; see the Perron-Frobenius theorem ), using any non-zero and non-negative initial vector to compute (repeated multiplication by ), then for any state variable ( th value of , we have an asymptotic growth rate matching : . Furthermore, if we find the sum of the state vectors, , and the calculate the stage distribution , then , an eigenvector whose terms add to 1 and .    A linear algebra course would teach us strategies to find eigenvalues and eigenvectors. We will let R calculate the values for us directly using the eigen command. Given a matrix M , the call eigen(M) will result in a list with two entries. The entry named values will contain numerical approximations of the eigenvalues (which might be complex numbers) and the entry named vectors will contain the eigenvectors in an order that matches the eigenvalues, stored as a matrix. The columns of that matrix will be the actual eigenvectors.  The following listing gives the code that will determine the eigenvalues and eigenvectors of the matrix we introduced in this section.    # Define the matrix M <- matrix(c(0.08, 0.12, 2, 3), nrow=2, ncol=2) my_eig <- eigen(M) print(my_eig$values) print(my_eig$vectors)     > print(my_eig$values) [1] 3.08 0.00 > print(my_eig$vectors) [,1] [,2] [1,] -0.5547002 -0.99920096 [2,] -0.8320503 0.03996804   We learn from this code that our matrix has two eigenvalues, and . The largest eigenvalue corresponds to which will be our dominant eigenvalue and precisely what we discovered earlier using the recursive sequence calculations. Because the dominant eigenvalue was the first value in the eigenvalue list, the corresponding 1st column of the eigenvector matrix will be one of the corresponding eigenvectors, . To find the stable stage distribution, we need to rescale this by finding the sum of entries and dividing that vector by the result.    # Extract the first column v <- my_eig$vectors[ ,1] # The total of the values is sum(v) # Rescale the vector stage_dist <- v \/ sum(v) print(stage_dist)     > print(stage_dist) [1] 0.4 0.6    You should not assume the first eigenvalue is automatically the dominant eigenvalue. Always check the sizes of all the eigenvalues before selecting the vector that will generate the stable stage distribution.   Because the state variables eventually start to act like they just multiply by the asymptotic growth rate , we can immediately deduce from the dominant eigenvalue of a projection matrix whether the population will grow ( ) or decay ( ).  "
},
{
  "id": "sec-structure-matrix-models-2",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  ...
  "level": "2",
  "url": "sec-structure-matrix-models.html#def-linear-combination",
  "type": "Definition",
  "number": "3.2.1",
  "title": "",
  "body": "  Given any mathematical objects that can be multiplied by constants and added together, say , and a corresponding number of constants, , the sum is called a linear combination of .   "
},
{
  "id": "sec-structure-matrix-models-5",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-5",
  "type": "Note",
  "number": "3.2.2",
  "title": "",
  "body": " If a particular state variable does not appear in the linear combination, its corresponding coefficient should be 0.  "
},
{
  "id": "sec-structure-matrix-models-6",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "matrix projection matrix linear algebra "
},
{
  "id": "sec-structure-matrix-models-7",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-7",
  "type": "Example",
  "number": "3.2.3",
  "title": "",
  "body": "  Consider a population of annual plants with a seedbank such that a seed in the seedbank has a probability 0.6 of germinating each year and a probability 0.2 of surviving in the seedbank to the next season, and where half of plants survive the growing season and subsequently produce an average of 10 seeds. Following the model development in , our projection model can be written . Our projection matrix becomes . If we define our state vector as , then the model can be written .  In R, we can create a matrix using the matrix command. This command expects data which will fill columns by default, and a specification of the number of rows and columns. The matrix above would be computed in R as matrix(c(0.08, 0.12, 2, 3), nrow=2, ncol=2) . Matrix-vector multiplication is performed using %*% . The following script illustrates how we would calculate sequence values with initial values .    # Define the projection matrix. M <- matrix(c(0.08, 0.12, 2, 3), nrow=2, ncol=2) # Initial values S0 <- 20 P0 <- 0 # Create space for the state vectors to be stored (as a matrix with 2 columns) N <- 30 X <- matrix(0, nrow=N+1, ncol=2) # Put initial value in 1st row # To refer to a row, leave column blank X[1, ] <- c(S0, P0) for (n in 1:N) { # Fill n+1 row of state values with project from n row of state values X[n+1, ] <- M %*% X[n, ] } library(tidyr) library(dplyr) # Generate a data-frame, calculate totals and stage distribution # S is in column 1 of X, P is in column 2 seedbank_data <- tibble(t = 0:N, S = X[ ,1], P = X[ ,2]) |> mutate(total = S + P) |> mutate(rho_S = S\/total, rho_P = P\/total) # Calculate growth ratio seedbank_data$lambda_S <- NA seedbank_data$lambda_P <- NA seedbank_data$lambda_S[2:(N+1)] <- seedbank_data$S[2:(N+1)] \/ seedbank_data$S[1:N] seedbank_data$lambda_P[2:(N+1)] <- seedbank_data$P[2:(N+1)] \/ seedbank_data$P[1:N]    After running this code, we discover that this population has an asymptotic growth ratio and a stable stage distribution with and .   "
},
{
  "id": "sec-structure-matrix-models-8",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "square matrix characteristic values eigenvalues eigenvector "
},
{
  "id": "thm-dominant-eigenvalue-asymptotic-rate",
  "level": "2",
  "url": "sec-structure-matrix-models.html#thm-dominant-eigenvalue-asymptotic-rate",
  "type": "Theorem",
  "number": "3.2.5",
  "title": "",
  "body": "  Let be a square matrix. The dominant eigenvalue of is the eigenvalue whose absolute value is the largest of all eigenvalues. Under typical conditions for population projection matrices (irreducible, aperiodic, and non-negative; see the Perron-Frobenius theorem ), using any non-zero and non-negative initial vector to compute (repeated multiplication by ), then for any state variable ( th value of , we have an asymptotic growth rate matching : . Furthermore, if we find the sum of the state vectors, , and the calculate the stage distribution , then , an eigenvector whose terms add to 1 and .   "
},
{
  "id": "sec-structure-matrix-models-12",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-12",
  "type": "Listing",
  "number": "3.2.6",
  "title": "",
  "body": "  # Define the matrix M <- matrix(c(0.08, 0.12, 2, 3), nrow=2, ncol=2) my_eig <- eigen(M) print(my_eig$values) print(my_eig$vectors)   "
},
{
  "id": "sec-structure-matrix-models-14",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-14",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "sec-structure-matrix-models-15",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-15",
  "type": "Listing",
  "number": "3.2.7",
  "title": "",
  "body": "  # Extract the first column v <- my_eig$vectors[ ,1] # The total of the values is sum(v) # Rescale the vector stage_dist <- v \/ sum(v) print(stage_dist)   "
},
{
  "id": "sec-structure-matrix-models-17",
  "level": "2",
  "url": "sec-structure-matrix-models.html#sec-structure-matrix-models-17",
  "type": "Note",
  "number": "3.2.8",
  "title": "",
  "body": " You should not assume the first eigenvalue is automatically the dominant eigenvalue. Always check the sizes of all the eigenvalues before selecting the vector that will generate the stable stage distribution.  "
},
{
  "id": "sec-structure-example-mosquitoes",
  "level": "1",
  "url": "sec-structure-example-mosquitoes.html",
  "type": "Section",
  "number": "3.3",
  "title": "Example: Mosquito Growth",
  "body": " Example: Mosquito Growth   Many insects exhibit distinct stages of development that are well modeled using structured populations. Females lay fertilized eggs which then take time to develop. When the eggs hatch, larvae emerge and often go through various larval stages of development. There is often a pupal stage in which a final metamorphosis takes place to an emerging adult. In addition, insects reproduce sexually, so it is often useful to distinguish male and female counts separately, at least at the adult stage.  For our illustrative example, we will consider the Aedes aegyptii mosquito (see ) that is most responsible for the spread of dengue fever, yellow fever, and chikungunya. Like other Aedes mosquito species, adult female mosquitoes find open still water and lay their eggs on a damp surface that is likely to be submerged by water. Once submerged, the egg will hatch after a few days and a larva will emerge in the water. These larvae swim in the water, eating algae and other smaller organisms in the water. As they develop and grow, the larvae transition between four different instar stages. After the fourth instar stage, the larva enters a pupal stage during which the individual metamorophoses to an adult. Each stage depends on the the presence of water and growth is accelerated with warmer temperatures.   Illustration of the life cycle of Aedes aegyptii mosquito. Image courtesy CDC, USA, .   Image of Aedes aegypti mosquito showing egg, larva, pupa, adult stages    In order to create a model for the mosquito population, let us identify some key assumptions. Because the mosquitos rely on water for their life cycle and temperatures control the rate of development, the population will naturally fluctuate based on the weather. Adult mosquitos are also able to fly, so their range could be impacted by migration from nearby areas. We will consider a simplified population with constantly available water supplies and steady temperatures. That is, we might suppose we are looking at optimal growth by the population under ideal conditions.  For a simplified model, we will consider only four stages eggs, larvae, pupae, and adults. In addition, at the adult stage for this model, we will only consider female adults since they are the only subpopulation that will actually produce eggs. If we wanted our model to include mating interactions in some way, then we would separately consider the male and female subpopulations. Development is measured in terms of days, so our model will use a time step of 1 day.   Model for a four-stage model of mosquito development.      Model Equations and Parameters  We now work to develop model parameters based on our reference material ( ). Under ideal conditions, we see that an egg should hatch in about two days. Larvae go through four instar stages, where the last stage is longest at about three days. We will assume that the average length of the larval stage is four days. The pupal stage takes about two days. The adults survive between two weeks to a month and are able to reproduce up to five times. Each batch of eggs is typically between 100 and 200 eggs. We do not have specific figures for mortality rates, which will depend on the local conditions.  How do we go from durations of stages to model transition rates? One approach would be to have a different stage for each age in a given stage. This would have two different egg stages. New eggs would all go in the first stage. All eggs that survived one day would transition to the second age stage. All eggs that survived the second day would transition to the first larval stage. This would be repeated until we reached the adult stage, requiring a total of 8 different developmental stages. Such an age-based model works well if timing is very clearly synchronized.  A second approach is to think of transitions happening randomly so that the average time of transition matches the reported values. We build from a mathematical property of a geometric random variable , which counts how many times we repeat a random test (like a coin flip) before an event occurs. If the probability of the event happening on a given trial is , then the average number of attempts before the first success occurs is given by . Consequently, if we to perform a daily test on whether to transition or not such that the average number of days spent prior to the transition is given by , then we want that daily test to occur with probability . This probability will then be the fraction of the number of individuals that experience the transition. This random approach using average times is the least-possibly synchronized model because each individual is effectively starting over every day with a fresh chance to transition.  We will introduce model parameters in the abstract and then assign specific values based on the given data. Suppose that eggs have an equal chance of dying each day, with a probability . The fraction of eggs that survive the day is therefore . Of these eggs, some transition to the larval stage with a probability while the remainder stay in the egg state another day. Based on the properties of a geometric random variable, we want where is the average duration of the egg state. In a similar way, larvae will have a probability of dying each day and a probability of transitioning to the pupal stage. For the same reason, pupae will have probabilities and for dying and transitioning, respectively. As noted, the mortality probabilities are specific to the environment and will not be found in the general literature.  Adults require a little extra thought. Only half of emerging adults will be female. (This might not even be the correct fraction an experimentally observed sex ratio would allow us to be more precise.) An adult can live between two weeks and a month but only reproduce up to five times. How can we deal with this? One approach is to spread out the total number of eggs across the entire lifespan of the adult, . That is, if is the number of eggs in a single laying and there are 5 reproductive episodes, then we have a daily average number of eggs per female given by . Adults will have a probability of expiring given by in order for adults to survive an average of days. Alternatively, we could introduce an extra adult stage during which all reproduction occurs and we contrive transition rates so that this stage is visited an average of 5 times and also so that the average number of days lived equals . This would require adults spend at least one day without reproducing before entering the state of laying eggs. A mathematical subject called recurrence theory allows us to calculate transition rates. For both adult states, we still want a probability of dying given by . However, we need nonreproducing adults to have a probability of transitioning to the reproductive state given by . Otherwise, adults will be in the non-reproductive state the next day.  Based on our analysis, we are ready to write down our model for the mosquito population. If we spread the reproduction over every day of an adult's life, the state variables will follow the recursive equations If instead we have two adult stages with reproduction only coming from , the recursive equations become     Simulation, Asymptotic Growth, and Stable Distributions  The two models are implemented in R in the following listings, initially using an adult lifespan of 28 days and an average number of 100 eggs per reproductive cycle. We start each model with 100 eggs and no mosquitoes in any other stage and follow the model for 90 days. We will graph the number of individuals in each stage and also calculate the daily growth factor where is the total population on day .    # Life cycle parameters T_egg <- 2 T_larva <- 4 T_pupa <- 2 T_adult <- 28 N_egg <- 100 # Model parameters # E,L,P mortality probabilities are arbitrarily set -- not based on life cycle m_E <- 0.05 m_L <- 0.05 m_P <- 0.05 p_E <- 1\/T_egg p_L <- 1\/T_larva p_P <- 1\/T_pupa e_A <- 1\/T_adult # Create space for data and fill with 0 maxT <- 90 t_seq <- 0:maxT E_seq <- double(maxT+1) L_seq <- double(maxT+1) P_seq <- double(maxT+1) A_seq <- double(maxT+1) # Initial values -- only eggs nonzero E_seq[1] <- N_egg # Now perform the projections for (t in 1:maxT) { E_seq[t+1] <- (1-m_E) * (1-p_E) * E_seq[t] + (5 * N_egg)\/T_adult * A_seq[t] L_seq[t+1] <- (1-m_E) * p_E * E_seq[t] + (1-m_L) * (1-p_L) * L_seq[t] P_seq[t+1] <- (1-m_L) * p_L * L_seq[t] + (1-m_P) * (1-p_P) * P_seq[t] A_seq[t+1] <- 0.5 * (1-m_P) * p_P * P_seq[t] + (1-e_A) * A_seq[t] } # Visualize dynamics --- expect exponential growth so use log-scale pop_growth <- ggplot() + # Add data; use color to label the different stages geom_point(mapping = aes(x = t_seq, y = E_seq, color = \"S1\")) + geom_point(mapping = aes(x = t_seq, y = L_seq, color = \"S2\")) + geom_point(mapping = aes(x = t_seq, y = P_seq, color = \"S3\")) + geom_point(mapping = aes(x = t_seq, y = A_seq, color = \"S4\")) + # Add legend information scale_color_discrete(breaks=c(\"S1\", \"S2\", \"S3\", \"S4\"), labels = c(\"eggs\",\"larvae\",\"pupae\",\"adult (F)\"), name=\"stage\") + # Add logarithimc scale scale_y_continuous(trans = \"log10\") + # Add labels labs(x = \"time\", y = \"population\") show(pop_growth) # Visualize the growth ratio T_seq <- E_seq + L_seq + P_seq + A_seq ratio_plot <- ggplot() + # Add data geom_point(mapping = aes(x = 1:maxT, T_seq[2:(maxT+1)] \/ T_seq[1:maxT])) + # Add labels labs(x = \"time\", y = \"lambda = T(t)\/T(t-1)\") show(ratio_plot)     Population values predicted by ELPA model using uniform birth rates   Plot of the different stages as functions of time     Growth ratio of ELPA model using uniform birth rates   Growth ratio values      # Life cycle parameters T_egg <- 2 T_larva <- 4 T_pupa <- 2 T_adult <- 28 N_egg <- 100 # Model parameters # E,L,P mortality probabilities are arbitrarily set -- not based on life cycle m_E <- 0.05 m_L <- 0.05 m_P <- 0.05 e_A <- 1\/T_adult p_E <- 1\/T_egg p_L <- 1\/T_larva p_P <- 1\/T_pupa p_A <- 5\/(T_adult - 5) # probability of entering reproductive cycle # Create space for data and fill with 0 maxT <- 90 t_seq <- 0:maxT E_seq <- double(maxT+1) L_seq <- double(maxT+1) P_seq <- double(maxT+1) A1_seq <- double(maxT+1) A2_seq <- double(maxT+1) # Initial values -- only eggs nonzero E_seq[1] <- N_egg # Now perform the projections for (t in 1:maxT) { E_seq[t+1] <- (1-m_E) * (1-p_E) * E_seq[t] + N_egg * A_seq[t] L_seq[t+1] <- (1-m_E) * p_E * E_seq[t] + (1-m_L) * (1-p_L) * L_seq[t] P_seq[t+1] <- (1-m_L) * p_L * L_seq[t] + (1-m_P) * (1-p_P) * P_seq[t] A1_seq[t+1] <- 0.5 * (1-m_P) * p_P * P_seq[t] + (1- e_A - p_A) * A1_seq[t] + (1-e_A) * A2_seq[t] A2_seq[t+1] <- p_A * A1_seq[t] } # Visualize dynamics --- expect exponential growth so use log-scale pop_growth <- ggplot() + # Add data; use color to label the different stages geom_point(mapping = aes(x = t_seq, y = E_seq, color = \"S1\")) + geom_point(mapping = aes(x = t_seq, y = L_seq, color = \"S2\")) + geom_point(mapping = aes(x = t_seq, y = P_seq, color = \"S3\")) + geom_point(mapping = aes(x = t_seq, y = A1_seq+A2_seq, color = \"S4\")) + # Add legend information scale_color_discrete(breaks=c(\"S1\", \"S2\", \"S3\", \"S4\"), labels = c(\"eggs\",\"larvae\",\"pupae\",\"adult (F)\"), name=\"stage\") + # Add logarithimc scale scale_y_continuous(trans = \"log10\") + # Add labels labs(x = \"time\", y = \"population\") show(pop_growth) # Visualize the growth ratio T_seq <- E_seq + L_seq + P_seq + A1_seq + A2_seq ratio_plot <- ggplot() + # Add data geom_point(mapping = aes(x = 1:maxT, T_seq[2:(maxT+1)] \/ T_seq[1:maxT])) + # Add labels labs(x = \"time\", y = \"lambda = T(t)\/T(t-1)\") show(ratio_plot)     Growth of ELPA model using uniform birth rates   Plot of the different stages as functions of time     Growth ratio of ELPA model using a reproductive adult subgroup   Growth ratio values    We should notice that the limiting value of , which is the asymptotic growth rate for the population, are slightly different between the two models. The model that distributed reproduciton over all time periods had a slightly higher value ( just above 1.5) compared to the model with a designated class of reproductive adults ( just below 1.5). This is comparable to compound interest where dividing growth into more frequent contributions results in larger overall exponential growth because the earlier and more frequent small deposits have a greater potential to grow over time.  We can recover these values theoretically by creating the corresponding matrix models for our recursive equations. In order to do this, we must be careful that we use the same ordering for coefficients in the columns as we use for the recursive models in the rows. That is, if our state variables follow the order , then our coefficients for each row must be put in the matrix for the variables in that same order. The matrix for the first model is given by . The second model has more state variables, and so has a larger matrix, .    # Create a projection matrix for each model # Include the parameters defined earlier M1 <- matrix(data = c( (1-m_E)*(1-p_E), (1-m_E)*p_E, 0, 0, # column 1 0, (1-m_L)*(1-p_L), (1-m_L)*p_L, 0, # column 2 0, 0, (1-m_P)*(1-p_P), 0.5*(1-m_P)*p_P, # column 3 5\/T_adult * N_egg, 0, 0, 1-e_A # column 4 ), nrow = 4, ncol = 4) M2 <- matrix(data = c( (1-m_E)*(1-p_E), (1-m_E)*p_E, 0, 0, 0, # column 1 0, (1-m_L)*(1-p_L), (1-m_L)*p_L, 0, 0, # column 2 0, 0, (1-m_P)*(1-p_P), 0.5*(1-m_P)*p_P, 0, # column 3 0, 0, 0, 1 - e_A - p_A, p_A, # column 4 N_egg, 0, 0, 1-e_A, 0 # column 5 ), nrow = 5, ncol = 5) print(M1) print(M2) # Find eigenvalues and eigenvectors eig_sys1 <- eigen(M1) eig_sys2 <- eigen(M2) # Display the eigenvalues print(eig_sys1$values) print(eig_sys2$values)    > print(M1) [,1] [,2] [,3] [,4] [1,] 0.475 0.0000 0.0000 17.8571429 [2,] 0.475 0.7125 0.0000 0.0000000 [3,] 0.000 0.2375 0.4750 0.0000000 [4,] 0.000 0.0000 0.2375 0.9642857 > print(M2) [,1] [,2] [,3] [,4] [,5] [1,] 0.475 0.0000 0.0000 0.0000000 100.0000000 [2,] 0.475 0.7125 0.0000 0.0000000 0.0000000 [3,] 0.000 0.2375 0.4750 0.0000000 0.0000000 [4,] 0.000 0.0000 0.2375 0.7468944 0.9642857 [5,] 0.000 0.0000 0.0000 0.2173913 0.0000000 > print(eig_sys1$values) [1] 1.5151824+0.0000000i 0.6546172+0.8071957i 0.6546172-0.8071957i -0.1976311+0.0000000i > print(eig_sys2$values) [1] 1.4542159+0.0000000i 0.7936544+0.7849968i 0.7936544-0.7849968i -0.3160652+0.4504780i [5] -0.3160652-0.4504780i    When we look at the eigenvalues that are displayed, we can see that they are displayed as complex numbers. For each of the matrices, the largest eigenvalue is the first one in the list. This is often the case but never guaranteed, so we always need to check before proceeding to the next step. For the first matrix, , the dominant eigenvalue is . Since the imaginary part of the number is 0, we can just write , which exactly corresponds to the asymptotic growth rate that we observed in the simulation. Similarly, the second matrix, , has a dominant eigenvalue which corresponds to the asymptotic growth of the second model.  Once we know which eigenvalue is dominant, we can extract the corresponding eigenvector to find the stable stage distributions.    # After the previous calculations, use the eigenvectors v1 <- eig_sys1$vectors[ ,1] # Take column 1 print(v1 \/ sum(v1)) v2 <- eig_sys2$vectors[ ,1] # Take column 1 print(v2 \/ sum(v2))    > print(v1 \/ sum(v1)) [1] 0.56018292+0i 0.33149711+0i 0.07568919+0i 0.03263077+0i > > v2 <- eig_sys2$vectors[ ,1] # Take column 1 > print(v2 \/ sum(v2)) [1] 0.534465662+0i 0.342275500+0i 0.083015838+0i 0.035009427+0i 0.005233573+0i    Because some of the eigenvalues and eigenvectors had imaginary parts, they are represented as all having imaginary parts even though the imaginary part is 0. We can reformat the numbers as real values using the Re command.    # After the previous calculations, use the eigenvectors v1 <- Re(eig_sys1$vectors[ ,1]) # Take column 1; force as real print(v1 \/ sum(v1)) v2 <- Re(eig_sys2$vectors[ ,1]) # Take column 1; force as real print(v2 \/ sum(v2))    > print(v1 \/ sum(v1)) [1] 0.56018292 0.33149711 0.07568919 0.03263077 > print(v2 \/ sum(v2)) [1] 0.534465662 0.342275500 0.083015838 0.035009427 0.005233573    Based on the calculated distributions, we see that the first model has a stable distribution with approximately 56.0% as eggs, 33.1% as larvae, 7.6% as pupae, and 3.3% as adult females. Note that our model was developed to ignore adult male mosquitoes. If we wanted a full count, we would need to double the adult population. In a similar way, the second model has a stable distribution with approximately 53.4% as eggs, 34.2% as larvae, 8.3% as pupae, and 3.5% as adult females that are not reproducing and 0.5% as adult females that are reproducing.    Exploring the Impact of Additional Mortality  When we implemented our basic model, we took laboratory lifespan of adults to find a probability of expiration . This was fundamentally different from our mortality and survival probabilities that we imposed on the egg, larva, and pupa stages. In those cases, we considered unknown environmental rates that impacted survival. For adults, we considered expiration (natural death) as one of three possible states to which an adult could transition.  Let us rethink adult dynamics by also including environmental mortality on adults by introducing a new mortality probability that represents a fraction of adults that die due to external environmental causes, which might include control efforts. Prior to making a transition, an adult must survive the day with probability . Consequently, for our first model involving uniform adult reproduction, will need to be replaced by . In a similar way, our second model involving a reproducing class would require changing and be rewritten as We may also need to adjust the fecundity term in or , depending on whether eggs are deposited before (no change required) or after survival of the day ( should be replaced by ). Eliminating the external environmental mortality by setting reverts the models to their original form.  Now that we have a proposed change to the model, how can we explore the impact of this change? Because there is no density dependence, the only fixed point for this system is with no mosquitoes. We can instead look at how changing the mortality impacts the asymptotic growth rate, which is calculated as the dominant eigenvalue to the projection matrix. Computationally, we would do this with a loop that goes through an array of possible values for . For each such value, we will construct a new matrix, calculate its eigenvalue, and identify the largest of the values. In order to track these results, we will record the asymptotic growth rate in a new array. Notice that we do not need to run simulations since the eigenvalue is a more direct method of finding this value. The code listed below implements this for the modified first model with uniform reproduction among all adults. In addition, we calculate the results for putting mortality both before and after egg deposition.    # Create the basic model parameters T_egg <- 2 T_larva <- 4 T_pupa <- 2 T_adult <- 28 N_egg <- 100 # Model parameters # E,L,P mortality probabilities are arbitrarily set -- not based on life cycle m_E <- 0.05 m_L <- 0.05 m_P <- 0.05 p_E <- 1\/T_egg p_L <- 1\/T_larva p_P <- 1\/T_pupa e_A <- 1\/T_adult # Create the array of possible value for m_A mortality_array <- seq(from=0, to=1, by=0.025) # Create an array to store our growth rate that is same length N_pts <- length(mortality_array) growth_rate_array1 <- double(N_pts) growth_rate_array2 <- double(N_pts) # Run through the values and make the calculations for (i in 1:N_pts) { # Get the current value m_A <- mortality_array[i] # 1st method: deposit eggs then survive # Create the projection matrix M <- matrix(data = c( (1-m_E)*(1-p_E), (1-m_E)*p_E, 0, 0, # column 1 0, (1-m_L)*(1-p_L), (1-m_L)*p_L, 0, # column 2 0, 0, (1-m_P)*(1-p_P), 0.5*(1-m_P)*p_P, # column 3 5\/T_adult * N_egg, 0, 0, (1-e_A)*(1-m_A) # column 4 ), nrow = 4, ncol = 4) # Compute eigenvalues and eigenvectors eig_sys <- eigen(M) # Identify the maximum eigenvalue (real part) max_location <- which.max(abs(eig_sys$values)) growth_rate_array1[i] <- Re(eig_sys$values[max_location]) # 2nd method: survive then deposit eggs # Create the projection matrix M <- matrix(data = c( (1-m_E)*(1-p_E), (1-m_E)*p_E, 0, 0, # column 1 0, (1-m_L)*(1-p_L), (1-m_L)*p_L, 0, # column 2 0, 0, (1-m_P)*(1-p_P), 0.5*(1-m_P)*p_P, # column 3 5\/T_adult * N_egg * (1-m_A), 0, 0, (1-e_A)*(1-m_A) # column 4 ), nrow = 4, ncol = 4) # Compute eigenvalues and eigenvectors eig_sys <- eigen(M) # Identify the maximum eigenvalue (real part) max_location <- which.max(abs(eig_sys$values)) growth_rate_array2[i] <- Re(eig_sys$values[max_location]) } # Visualize the results library(ggplot2) ggplot() + geom_line(mapping = aes(x = mortality_array, y = growth_rate_array1, color = \"1\")) + geom_line(mapping = aes(x = mortality_array, y = growth_rate_array2, color = \"2\")) + scale_color_discrete(breaks=c(\"1\", \"2\"), labels = c(\"eggs first\",\"survive first\"), name=\"ordering\") + labs(x = \"mu_A (adult mortality)\", y = \"lambda (asymptotic growth rate)\")     Asymptotic growth rate as a function of adult mortality .   Plot showing asymptotic growth rate as it depends on adult mortality    The results in show a fundamental difference between having mortality before or after eggs are deposited. For this model, if all adults have the ability to deposit the designated fraction of eggs before needing to survive the day, the asymptotic growth rate remains above for all values of . There would always be enough eggs deposited to see the mosquito population grow even if all adults died after the first day ( ). On the other hand, if adults need to survive before they can deposit eggs, high values of result in and the population would decline.   "
},
{
  "id": "fig-Aedes-lifecycle",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#fig-Aedes-lifecycle",
  "type": "Figure",
  "number": "3.3.1",
  "title": "",
  "body": " Illustration of the life cycle of Aedes aegyptii mosquito. Image courtesy CDC, USA, .   Image of Aedes aegypti mosquito showing egg, larva, pupa, adult stages   "
},
{
  "id": "fig-ELPA-mosquito-model",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#fig-ELPA-mosquito-model",
  "type": "Figure",
  "number": "3.3.2",
  "title": "",
  "body": " Model for a four-stage model of mosquito development.   "
},
{
  "id": "sec-structure-example-mosquitoes-3-4",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#sec-structure-example-mosquitoes-3-4",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "geometric random variable "
},
{
  "id": "sec-structure-example-mosquitoes-3-6",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#sec-structure-example-mosquitoes-3-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "recurrence theory "
},
{
  "id": "listing-ELPA-uniform-reproduction",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#listing-ELPA-uniform-reproduction",
  "type": "Listing",
  "number": "3.3.3",
  "title": "",
  "body": "  # Life cycle parameters T_egg <- 2 T_larva <- 4 T_pupa <- 2 T_adult <- 28 N_egg <- 100 # Model parameters # E,L,P mortality probabilities are arbitrarily set -- not based on life cycle m_E <- 0.05 m_L <- 0.05 m_P <- 0.05 p_E <- 1\/T_egg p_L <- 1\/T_larva p_P <- 1\/T_pupa e_A <- 1\/T_adult # Create space for data and fill with 0 maxT <- 90 t_seq <- 0:maxT E_seq <- double(maxT+1) L_seq <- double(maxT+1) P_seq <- double(maxT+1) A_seq <- double(maxT+1) # Initial values -- only eggs nonzero E_seq[1] <- N_egg # Now perform the projections for (t in 1:maxT) { E_seq[t+1] <- (1-m_E) * (1-p_E) * E_seq[t] + (5 * N_egg)\/T_adult * A_seq[t] L_seq[t+1] <- (1-m_E) * p_E * E_seq[t] + (1-m_L) * (1-p_L) * L_seq[t] P_seq[t+1] <- (1-m_L) * p_L * L_seq[t] + (1-m_P) * (1-p_P) * P_seq[t] A_seq[t+1] <- 0.5 * (1-m_P) * p_P * P_seq[t] + (1-e_A) * A_seq[t] } # Visualize dynamics --- expect exponential growth so use log-scale pop_growth <- ggplot() + # Add data; use color to label the different stages geom_point(mapping = aes(x = t_seq, y = E_seq, color = \"S1\")) + geom_point(mapping = aes(x = t_seq, y = L_seq, color = \"S2\")) + geom_point(mapping = aes(x = t_seq, y = P_seq, color = \"S3\")) + geom_point(mapping = aes(x = t_seq, y = A_seq, color = \"S4\")) + # Add legend information scale_color_discrete(breaks=c(\"S1\", \"S2\", \"S3\", \"S4\"), labels = c(\"eggs\",\"larvae\",\"pupae\",\"adult (F)\"), name=\"stage\") + # Add logarithimc scale scale_y_continuous(trans = \"log10\") + # Add labels labs(x = \"time\", y = \"population\") show(pop_growth) # Visualize the growth ratio T_seq <- E_seq + L_seq + P_seq + A_seq ratio_plot <- ggplot() + # Add data geom_point(mapping = aes(x = 1:maxT, T_seq[2:(maxT+1)] \/ T_seq[1:maxT])) + # Add labels labs(x = \"time\", y = \"lambda = T(t)\/T(t-1)\") show(ratio_plot)   "
},
{
  "id": "fig-ELPA-uniform-population",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#fig-ELPA-uniform-population",
  "type": "Figure",
  "number": "3.3.4",
  "title": "",
  "body": " Population values predicted by ELPA model using uniform birth rates   Plot of the different stages as functions of time   "
},
{
  "id": "fig-ELPA-uniform-population-lambda",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#fig-ELPA-uniform-population-lambda",
  "type": "Figure",
  "number": "3.3.5",
  "title": "",
  "body": " Growth ratio of ELPA model using uniform birth rates   Growth ratio values   "
},
{
  "id": "listing-ELPA-reproductive-adults",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#listing-ELPA-reproductive-adults",
  "type": "Listing",
  "number": "3.3.6",
  "title": "",
  "body": "  # Life cycle parameters T_egg <- 2 T_larva <- 4 T_pupa <- 2 T_adult <- 28 N_egg <- 100 # Model parameters # E,L,P mortality probabilities are arbitrarily set -- not based on life cycle m_E <- 0.05 m_L <- 0.05 m_P <- 0.05 e_A <- 1\/T_adult p_E <- 1\/T_egg p_L <- 1\/T_larva p_P <- 1\/T_pupa p_A <- 5\/(T_adult - 5) # probability of entering reproductive cycle # Create space for data and fill with 0 maxT <- 90 t_seq <- 0:maxT E_seq <- double(maxT+1) L_seq <- double(maxT+1) P_seq <- double(maxT+1) A1_seq <- double(maxT+1) A2_seq <- double(maxT+1) # Initial values -- only eggs nonzero E_seq[1] <- N_egg # Now perform the projections for (t in 1:maxT) { E_seq[t+1] <- (1-m_E) * (1-p_E) * E_seq[t] + N_egg * A_seq[t] L_seq[t+1] <- (1-m_E) * p_E * E_seq[t] + (1-m_L) * (1-p_L) * L_seq[t] P_seq[t+1] <- (1-m_L) * p_L * L_seq[t] + (1-m_P) * (1-p_P) * P_seq[t] A1_seq[t+1] <- 0.5 * (1-m_P) * p_P * P_seq[t] + (1- e_A - p_A) * A1_seq[t] + (1-e_A) * A2_seq[t] A2_seq[t+1] <- p_A * A1_seq[t] } # Visualize dynamics --- expect exponential growth so use log-scale pop_growth <- ggplot() + # Add data; use color to label the different stages geom_point(mapping = aes(x = t_seq, y = E_seq, color = \"S1\")) + geom_point(mapping = aes(x = t_seq, y = L_seq, color = \"S2\")) + geom_point(mapping = aes(x = t_seq, y = P_seq, color = \"S3\")) + geom_point(mapping = aes(x = t_seq, y = A1_seq+A2_seq, color = \"S4\")) + # Add legend information scale_color_discrete(breaks=c(\"S1\", \"S2\", \"S3\", \"S4\"), labels = c(\"eggs\",\"larvae\",\"pupae\",\"adult (F)\"), name=\"stage\") + # Add logarithimc scale scale_y_continuous(trans = \"log10\") + # Add labels labs(x = \"time\", y = \"population\") show(pop_growth) # Visualize the growth ratio T_seq <- E_seq + L_seq + P_seq + A1_seq + A2_seq ratio_plot <- ggplot() + # Add data geom_point(mapping = aes(x = 1:maxT, T_seq[2:(maxT+1)] \/ T_seq[1:maxT])) + # Add labels labs(x = \"time\", y = \"lambda = T(t)\/T(t-1)\") show(ratio_plot)   "
},
{
  "id": "fig-ELPA-reproductive-adults",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#fig-ELPA-reproductive-adults",
  "type": "Figure",
  "number": "3.3.7",
  "title": "",
  "body": " Growth of ELPA model using uniform birth rates   Plot of the different stages as functions of time   "
},
{
  "id": "fig-ELPA-reproductive-adults-lambda",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#fig-ELPA-reproductive-adults-lambda",
  "type": "Figure",
  "number": "3.3.8",
  "title": "",
  "body": " Growth ratio of ELPA model using a reproductive adult subgroup   Growth ratio values   "
},
{
  "id": "sec-structure-example-mosquitoes-4-11",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#sec-structure-example-mosquitoes-4-11",
  "type": "Listing",
  "number": "3.3.9",
  "title": "",
  "body": "  # Create a projection matrix for each model # Include the parameters defined earlier M1 <- matrix(data = c( (1-m_E)*(1-p_E), (1-m_E)*p_E, 0, 0, # column 1 0, (1-m_L)*(1-p_L), (1-m_L)*p_L, 0, # column 2 0, 0, (1-m_P)*(1-p_P), 0.5*(1-m_P)*p_P, # column 3 5\/T_adult * N_egg, 0, 0, 1-e_A # column 4 ), nrow = 4, ncol = 4) M2 <- matrix(data = c( (1-m_E)*(1-p_E), (1-m_E)*p_E, 0, 0, 0, # column 1 0, (1-m_L)*(1-p_L), (1-m_L)*p_L, 0, 0, # column 2 0, 0, (1-m_P)*(1-p_P), 0.5*(1-m_P)*p_P, 0, # column 3 0, 0, 0, 1 - e_A - p_A, p_A, # column 4 N_egg, 0, 0, 1-e_A, 0 # column 5 ), nrow = 5, ncol = 5) print(M1) print(M2) # Find eigenvalues and eigenvectors eig_sys1 <- eigen(M1) eig_sys2 <- eigen(M2) # Display the eigenvalues print(eig_sys1$values) print(eig_sys2$values)    > print(M1) [,1] [,2] [,3] [,4] [1,] 0.475 0.0000 0.0000 17.8571429 [2,] 0.475 0.7125 0.0000 0.0000000 [3,] 0.000 0.2375 0.4750 0.0000000 [4,] 0.000 0.0000 0.2375 0.9642857 > print(M2) [,1] [,2] [,3] [,4] [,5] [1,] 0.475 0.0000 0.0000 0.0000000 100.0000000 [2,] 0.475 0.7125 0.0000 0.0000000 0.0000000 [3,] 0.000 0.2375 0.4750 0.0000000 0.0000000 [4,] 0.000 0.0000 0.2375 0.7468944 0.9642857 [5,] 0.000 0.0000 0.0000 0.2173913 0.0000000 > print(eig_sys1$values) [1] 1.5151824+0.0000000i 0.6546172+0.8071957i 0.6546172-0.8071957i -0.1976311+0.0000000i > print(eig_sys2$values) [1] 1.4542159+0.0000000i 0.7936544+0.7849968i 0.7936544-0.7849968i -0.3160652+0.4504780i [5] -0.3160652-0.4504780i   "
},
{
  "id": "sec-structure-example-mosquitoes-4-14",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#sec-structure-example-mosquitoes-4-14",
  "type": "Listing",
  "number": "3.3.10",
  "title": "",
  "body": "  # After the previous calculations, use the eigenvectors v1 <- eig_sys1$vectors[ ,1] # Take column 1 print(v1 \/ sum(v1)) v2 <- eig_sys2$vectors[ ,1] # Take column 1 print(v2 \/ sum(v2))    > print(v1 \/ sum(v1)) [1] 0.56018292+0i 0.33149711+0i 0.07568919+0i 0.03263077+0i > > v2 <- eig_sys2$vectors[ ,1] # Take column 1 > print(v2 \/ sum(v2)) [1] 0.534465662+0i 0.342275500+0i 0.083015838+0i 0.035009427+0i 0.005233573+0i   "
},
{
  "id": "sec-structure-example-mosquitoes-4-16",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#sec-structure-example-mosquitoes-4-16",
  "type": "Listing",
  "number": "3.3.11",
  "title": "",
  "body": "  # After the previous calculations, use the eigenvectors v1 <- Re(eig_sys1$vectors[ ,1]) # Take column 1; force as real print(v1 \/ sum(v1)) v2 <- Re(eig_sys2$vectors[ ,1]) # Take column 1; force as real print(v2 \/ sum(v2))    > print(v1 \/ sum(v1)) [1] 0.56018292 0.33149711 0.07568919 0.03263077 > print(v2 \/ sum(v2)) [1] 0.534465662 0.342275500 0.083015838 0.035009427 0.005233573   "
},
{
  "id": "sec-structure-example-mosquitoes-5-5",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#sec-structure-example-mosquitoes-5-5",
  "type": "Listing",
  "number": "3.3.12",
  "title": "",
  "body": "  # Create the basic model parameters T_egg <- 2 T_larva <- 4 T_pupa <- 2 T_adult <- 28 N_egg <- 100 # Model parameters # E,L,P mortality probabilities are arbitrarily set -- not based on life cycle m_E <- 0.05 m_L <- 0.05 m_P <- 0.05 p_E <- 1\/T_egg p_L <- 1\/T_larva p_P <- 1\/T_pupa e_A <- 1\/T_adult # Create the array of possible value for m_A mortality_array <- seq(from=0, to=1, by=0.025) # Create an array to store our growth rate that is same length N_pts <- length(mortality_array) growth_rate_array1 <- double(N_pts) growth_rate_array2 <- double(N_pts) # Run through the values and make the calculations for (i in 1:N_pts) { # Get the current value m_A <- mortality_array[i] # 1st method: deposit eggs then survive # Create the projection matrix M <- matrix(data = c( (1-m_E)*(1-p_E), (1-m_E)*p_E, 0, 0, # column 1 0, (1-m_L)*(1-p_L), (1-m_L)*p_L, 0, # column 2 0, 0, (1-m_P)*(1-p_P), 0.5*(1-m_P)*p_P, # column 3 5\/T_adult * N_egg, 0, 0, (1-e_A)*(1-m_A) # column 4 ), nrow = 4, ncol = 4) # Compute eigenvalues and eigenvectors eig_sys <- eigen(M) # Identify the maximum eigenvalue (real part) max_location <- which.max(abs(eig_sys$values)) growth_rate_array1[i] <- Re(eig_sys$values[max_location]) # 2nd method: survive then deposit eggs # Create the projection matrix M <- matrix(data = c( (1-m_E)*(1-p_E), (1-m_E)*p_E, 0, 0, # column 1 0, (1-m_L)*(1-p_L), (1-m_L)*p_L, 0, # column 2 0, 0, (1-m_P)*(1-p_P), 0.5*(1-m_P)*p_P, # column 3 5\/T_adult * N_egg * (1-m_A), 0, 0, (1-e_A)*(1-m_A) # column 4 ), nrow = 4, ncol = 4) # Compute eigenvalues and eigenvectors eig_sys <- eigen(M) # Identify the maximum eigenvalue (real part) max_location <- which.max(abs(eig_sys$values)) growth_rate_array2[i] <- Re(eig_sys$values[max_location]) } # Visualize the results library(ggplot2) ggplot() + geom_line(mapping = aes(x = mortality_array, y = growth_rate_array1, color = \"1\")) + geom_line(mapping = aes(x = mortality_array, y = growth_rate_array2, color = \"2\")) + scale_color_discrete(breaks=c(\"1\", \"2\"), labels = c(\"eggs first\",\"survive first\"), name=\"ordering\") + labs(x = \"mu_A (adult mortality)\", y = \"lambda (asymptotic growth rate)\")   "
},
{
  "id": "fig-ELPA-adult-mortality-exploration",
  "level": "2",
  "url": "sec-structure-example-mosquitoes.html#fig-ELPA-adult-mortality-exploration",
  "type": "Figure",
  "number": "3.3.13",
  "title": "",
  "body": " Asymptotic growth rate as a function of adult mortality .   Plot showing asymptotic growth rate as it depends on adult mortality   "
},
{
  "id": "sec-structure-density-dependent",
  "level": "1",
  "url": "sec-structure-density-dependent.html",
  "type": "Section",
  "number": "3.4",
  "title": "Density Dependence in Structured Populations",
  "body": " Density Dependence in Structured Populations   In our analysis of the mosquito population using a structured model, like all linear projection models, the dynamics eventually resulted in either exponential growth or exponential decay. Just as with simple populations, exponential growth is not a valid model for structured populations when the population becomes larger than the environment is capable of supporting. There must be a change to the model when the population starts to become sufficiently large.  For the mosquito population, one possible bottleneck is the water where larvae develop. As more larvae grow in the fixed number of open water sources, they will compete for food and space. There may also be limited space for female mosquitoes to deposit their eggs. Even beyond these considerations, a high mosquito population might also attract predators that might put additional pressure on the population.    Model Development  We will choose just one pressure point on the population and add density-dependence in the model at the larva stage. Specifically, we could consider the mortality rate for larvae to be an increasing function of the number of larvae present. A mortality rate in this context is a probability and so is constrained to be between 0 and 1. We will introduce a mortality function that is an increasing function of with a horizontal asymptote as . Graphically, we want a function that looks something like the graph in .   Graph of a prototypical density-dependent mortality probability.   density-dependent mortality probability function    From calculus, we know that a function of the form will have a horizontal asymptote as . We also know that . Calculating , we find so that . By choosing appropriate values for and , we can therefore make our graph match an initial mortality rate and for small values of have additional mortality that is proportional to (density-dependence) with proportionality constant (slope) by choosing and .  Using this mortality rate for larvae, and the uniform reproductive rate for adults, we obtain a modified density-dependent mosquito model. Because the larvae mortality rate in and always appeared as , we will rewrite this first. We now know that is a function of with so that we can write Substituting this result into and , our system of projection equations then become (using in place of )     Simulation  Because we have introduced density dependence, the model is no longer a linear model. That is, there is at least one projection equation that is not just a linear combination of the state variables. The analysis that we did previously works well for small mosquito populations when , but as the population gets larger, the density dependence slows the exponential growth and we might approach an equilibrium.  We can explore this initially using simulation. We will use the same parameters as in the previous section, with the exception that we need to introduce our density dependent parameter , representing the increase in larvae mortality per individual in the population. In our earlier simulation, we used as an arbitrarily chosen value to reflect some environmental daily loss of larvae. Suppose we want our model to increase mortality to if the larvae population in the water reaches . This represents and increase of for a change , so our parameter .    # Life cycle parameters T_egg <- 2 T_larva <- 4 T_pupa <- 2 T_adult <- 28 N_egg <- 100 # Model parameters # E,L,P mortality probabilities are arbitrarily set -- not based on life cycle m_E <- 0.05 m_L0 <- 0.05 k <- 0.0005 # density dependent increase in mortality m_P <- 0.05 p_E <- 1\/T_egg p_L <- 1\/T_larva p_P <- 1\/T_pupa e_A <- 1\/T_adult # Create space for data and fill with 0 maxT <- 90 t_seq <- 0:maxT E_seq <- double(maxT+1) L_seq <- double(maxT+1) P_seq <- double(maxT+1) A_seq <- double(maxT+1) # Initial values -- only eggs nonzero E_seq[1] <- N_egg # Now perform the projections for (t in 1:maxT) { E_seq[t+1] <- (1-m_E) * (1-p_E) * E_seq[t] + (5 * N_egg)\/T_adult * A_seq[t] L_seq[t+1] <- (1-m_E) * p_E * E_seq[t] + (1-m_L0)^2 * (1-p_L) * L_seq[t] \/ (k * L_seq[t] + 1 - m_L0) P_seq[t+1] <- (1-m_L0)^2 * p_L * L_seq[t] \/ (k * L_seq[t] + 1 - m_L0) + (1-m_P) * (1-p_P) * P_seq[t] A_seq[t+1] <- 0.5 * (1-m_P) * p_P * P_seq[t] + (1-e_A) * A_seq[t] } # Visualize dynamics --- expect exponential growth so use log-scale library(ggplot2) pop_growth <- ggplot() + # Add data; use color to label the different stages geom_point(mapping = aes(x = t_seq, y = E_seq, color = \"1\")) + geom_point(mapping = aes(x = t_seq, y = L_seq, color = \"2\")) + geom_point(mapping = aes(x = t_seq, y = P_seq, color = \"3\")) + geom_point(mapping = aes(x = t_seq, y = A_seq, color = \"4\")) + # Add legend information scale_color_discrete(breaks=c(\"1\", \"2\", \"3\", \"4\"), labels = c(\"eggs\",\"larvae\",\"pupae\",\"adult (F)\"), name=\"stage\") + # Add logarithimc scale scale_y_continuous(trans = \"log10\") + # Add labels labs(x = \"time\", y = \"population\") show(pop_growth)     Population values predicted by the density-dependent ELPA model   Plot of the different stages as functions of time    We can see that the mosquito population starts out like it will grow exponentially, but after only a few days the population values start to level out. Our population is converging toward an equilibrium. By inspecting the final recorded values for each of the state variables, we can see that our simulation has not quite reached equilibrium for . Increasing our final time value to , we still have not reached equilibrium (values are still changing), but we can see we are close with values . To find actual equilibrium values, we need to do some analysis.    Finding Fixed Points for Structured Populations  Recall that for a single sequence with projection function , we find a fixed point by solving the equation . When working with structured populations, we need all of the projection equations to be at an equilibrium. That is, we need to find values so that , , and .  We will need to solve four equations involving four variables at the same time. In an abstract sense, if we thought of our four projection formulas as four different projection functions that depend on the full state of the population, then we are trying to solve the following four equations simultaneously,   Rather than work through the mathematical theory for how to do this solving of equations by hand, we will let a computer algebra system do the work for us (if possible). An example of how we would try to do this using Sage is illustrated below. If we want symbolic solutions, we need to declare every parameter as a symbol. This often slows the computation down or might even make it so the algebra system can't find the answer. Instead, we can declare the values directly and then solve. Only the second method is illustrated here.   There are two equilibria or fixed point states. A total absence of mosquitoes is one of the solutions, . The second equilbrium state is what our simulation was approaching, . Our simulation gives us strong reason to believe that the second fixed point is stable while the fixed point with no mosquitoes is unstable.    Partial Derivatives  Recall that for simple projection models, , where is a fixed point, we tested the local stability of the fixed point by looking at the value of the slope . For structured populations, we will do a similar thing. However, in this case, there are more functions and more variables. We will be creating a matrix full of derivatives of each of the functions with respect to each of the variables. In place of the single slope to determine stability, we will be looking at the dominant eigenvalue of this matrix.  We first need to introduce the idea of a partial derivative . When a function is defined involving multiple variables, such as , a partial derivative gives us the rate of change of this function when only one of the variables is changed. We can not use the prime notation, as in because that doesn't tell us which variable was changed. Instead, we use a subscript indicating the variable, so that represents the rate of change when is changing and represents the rate of change when is changing. The derivative operator in calculus is also updated to use curly d to indicate we are holding other variables constant so that we have and . These are also sometimes abbreviated as operators to also involve subscripts and .    If , find and .    The partial derivative finds the derivative treating as the variable and as if it were constant. Whenever a term has just , that entire term is treated as if it were constant. And if a term has a formula involving as a factor, that entire factor is treated like a constant multiple. Consequently,   The partial derivative does the reverse we treat as the variable and any term involving as if that were a constant. Thus,   We can use a computer algebra system to perform the derivatives by specifying the variable of differentiation.     Our abstract setup for the matrix of derivatives presumes that have state variables ordered . Each state variable has its own projection function involving potentially any of the state variables, . For each function , we will compute all of the partial derivatives with respect to , , etc., one at a time, and put them in the th row of a matrix. This matrix full of partial derivatives is named the Jacobian matrix, or more simply the derivative matrix. The order is critical in that the first column will have all of the partial derivatives with respect to , the second column will have all of the partial derivatives with respect to , and so on. I prefer the notation for the derivative matrix.    Suppose we started with a projection model for given by recursive formulas This model could represent a simplified density-dependent seed-bank annual plant model. The parameter represents the fraction of seeds from the seed bank that germinate while represents the fraction of non-germinating seeds that will survive to the next year. Plants are modeled with density dependence so that the number of seeds per plant starts at but decreases with increasing population sizes using the per plant seed model .  Having specified an order for the state variables of , we can define the two projection functions . We can compute the partial derivatives, following the same order of first and then : .  The Jacobian or derivative matrix is found by putting the partial derivatives of in the first row and the partial derivatives of in the second row. Notice that this formula still depends on the value of . This is because our original function was nonlinear with respect to .  This example focused only on finding the Jacobian matrix. We would need to calculate fixed points and then evaluate this matrix using the fixed point state values before proceeding further.    A computer algebra system can automate this process for us. To do this, we must have a way to inform the system of the ordering for the variables as well as for the projection functions. A possible approach using Sage is provided below, using the previous example involving . To modify this, you only need to update the names of the state variables and paramters, define your projection functions, and then update the state_vars and projections lists to match order.     Testing the Stability of a Fixed Point  To test the local stability of a fixed point in a structured population, we take advantage of the following theorem.    Suppose a discrete-time system has a state vector governed by projection functions has a fixed point  satisfying . Calculate the Jacobian matrix evaluated at and then find the dominant eigenvalue of that matrix .   If , then is locally unstable .    If , then is locally stable .   If , the test is inconclusive.    Since we are already using the computer algebra system Sage to generate our matrix and find our fixed points, we might as well also ask this system to calculate our eigenvalues. Because Sage is designed to accommodate many mathematical subtleties, the command is slightly more complex than the eigen command in R. We will illustrate the analysis for our simple seed model introduced above.    For the density-dependent seed bank model we will use Sage to find the fixed points and calculate the Jacobian matrix using the fixed point solutions. Suppose that we are working with parameter values (50% survival of seeds to next year), (60% germination rate each year), (100 seeds per plant in ideal conditions), and (controls how fast density dependence impacts the seed production). We borrow the derivative code from the earlier Sage calculation and add in the calculation of the fixed points.   When evaluating the Sage code, we discover that there are two fixed point solutions. The first fixed point listed is . The Jacobian matrix at this fixed point is given by . Finally, Sage reports the eigenvalues as and . The dominant eigenvalue is the eigenvalue with largest magnitude . Since this eigenvalue has magnitude smaller than 1, the fixed point is stable.  The second fixed point is . The Jacobian matrix at this fixed point is given by and Sage reports the eigenvalues as and . Because the dominant eigenvalue, , has magnitude greater than 1, the fixed point is unstable.     "
},
{
  "id": "fig-density-dependent-mortality",
  "level": "2",
  "url": "sec-structure-density-dependent.html#fig-density-dependent-mortality",
  "type": "Figure",
  "number": "3.4.1",
  "title": "",
  "body": " Graph of a prototypical density-dependent mortality probability.   density-dependent mortality probability function   "
},
{
  "id": "listing-ELPA-density-dependent",
  "level": "2",
  "url": "sec-structure-density-dependent.html#listing-ELPA-density-dependent",
  "type": "Listing",
  "number": "3.4.2",
  "title": "",
  "body": "  # Life cycle parameters T_egg <- 2 T_larva <- 4 T_pupa <- 2 T_adult <- 28 N_egg <- 100 # Model parameters # E,L,P mortality probabilities are arbitrarily set -- not based on life cycle m_E <- 0.05 m_L0 <- 0.05 k <- 0.0005 # density dependent increase in mortality m_P <- 0.05 p_E <- 1\/T_egg p_L <- 1\/T_larva p_P <- 1\/T_pupa e_A <- 1\/T_adult # Create space for data and fill with 0 maxT <- 90 t_seq <- 0:maxT E_seq <- double(maxT+1) L_seq <- double(maxT+1) P_seq <- double(maxT+1) A_seq <- double(maxT+1) # Initial values -- only eggs nonzero E_seq[1] <- N_egg # Now perform the projections for (t in 1:maxT) { E_seq[t+1] <- (1-m_E) * (1-p_E) * E_seq[t] + (5 * N_egg)\/T_adult * A_seq[t] L_seq[t+1] <- (1-m_E) * p_E * E_seq[t] + (1-m_L0)^2 * (1-p_L) * L_seq[t] \/ (k * L_seq[t] + 1 - m_L0) P_seq[t+1] <- (1-m_L0)^2 * p_L * L_seq[t] \/ (k * L_seq[t] + 1 - m_L0) + (1-m_P) * (1-p_P) * P_seq[t] A_seq[t+1] <- 0.5 * (1-m_P) * p_P * P_seq[t] + (1-e_A) * A_seq[t] } # Visualize dynamics --- expect exponential growth so use log-scale library(ggplot2) pop_growth <- ggplot() + # Add data; use color to label the different stages geom_point(mapping = aes(x = t_seq, y = E_seq, color = \"1\")) + geom_point(mapping = aes(x = t_seq, y = L_seq, color = \"2\")) + geom_point(mapping = aes(x = t_seq, y = P_seq, color = \"3\")) + geom_point(mapping = aes(x = t_seq, y = A_seq, color = \"4\")) + # Add legend information scale_color_discrete(breaks=c(\"1\", \"2\", \"3\", \"4\"), labels = c(\"eggs\",\"larvae\",\"pupae\",\"adult (F)\"), name=\"stage\") + # Add logarithimc scale scale_y_continuous(trans = \"log10\") + # Add labels labs(x = \"time\", y = \"population\") show(pop_growth)   "
},
{
  "id": "fig-ELPA-density-dependent-population",
  "level": "2",
  "url": "sec-structure-density-dependent.html#fig-ELPA-density-dependent-population",
  "type": "Figure",
  "number": "3.4.3",
  "title": "",
  "body": " Population values predicted by the density-dependent ELPA model   Plot of the different stages as functions of time   "
},
{
  "id": "subsec-structure-partial-derivatives-3",
  "level": "2",
  "url": "sec-structure-density-dependent.html#subsec-structure-partial-derivatives-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsec-structure-partial-derivatives-4",
  "level": "2",
  "url": "sec-structure-density-dependent.html#subsec-structure-partial-derivatives-4",
  "type": "Example",
  "number": "3.4.4",
  "title": "",
  "body": "  If , find and .    The partial derivative finds the derivative treating as the variable and as if it were constant. Whenever a term has just , that entire term is treated as if it were constant. And if a term has a formula involving as a factor, that entire factor is treated like a constant multiple. Consequently,   The partial derivative does the reverse we treat as the variable and any term involving as if that were a constant. Thus,   We can use a computer algebra system to perform the derivatives by specifying the variable of differentiation.    "
},
{
  "id": "subsec-structure-partial-derivatives-5",
  "level": "2",
  "url": "sec-structure-density-dependent.html#subsec-structure-partial-derivatives-5",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Jacobian "
...
{
  "id": "subsec-structure-partial-derivatives-6",
  "level": "2",
  "url": "sec-structure-density-dependent.html#subsec-structure-partial-derivatives-6",
  "type": "Example",
  "number": "3.4.5",
  "title": "",
  "body": "  Suppose we started with a projection model for given by recursive formulas This model could represent a simplified density-dependent seed-bank annual plant model. The parameter represents the fraction of seeds from the seed bank that germinate while represents the fraction of non-germinating seeds that will survive to the next year. Plants are modeled with density dependence so that the number of seeds per plant starts at but decreases with increasing population sizes using the per plant seed model .  Having specified an order for the state variables of , we can define the two projection functions . We can compute the partial derivatives, following the same order of first and then : .  The Jacobian or derivative matrix is found by putting the partial derivatives of in the first row and the partial derivatives of in the second row. Notice that this formula still depends on the value of . This is because our original function was nonlinear with respect to .  This example focused only on finding the Jacobian matrix. We would need to calculate fixed points and then evaluate this matrix using the fixed point state values before proceeding further.   "
},
{
  "id": "thm-discrete-dynamics-local-stability",
  "level": "2",
  "url": "sec-structure-density-dependent.html#thm-discrete-dynamics-local-stability",
  "type": "Theorem",
  "number": "3.4.6",
  "title": "",
  "body": "  Suppose a discrete-time system has a state vector governed by projection functions has a fixed point  satisfying . Calculate the Jacobian matrix evaluated at and then find the dominant eigenvalue of that matrix .   If , then is locally unstable .    If , then is locally stable .   If , the test is inconclusive.   "
},
{
  "id": "subsec-structure-fixed-point-stability-5",
  "level": "2",
  "url": "sec-structure-density-dependent.html#subsec-structure-fixed-point-stability-5",
  "type": "Example",
  "number": "3.4.7",
  "title": "",
  "body": "  For the density-dependent seed bank model we will use Sage to find the fixed points and calculate the Jacobian matrix using the fixed point solutions. Suppose that we are working with parameter values (50% survival of seeds to next year), (60% germination rate each year), (100 seeds per plant in ideal conditions), and (controls how fast density dependence impacts the seed production). We borrow the derivative code from the earlier Sage calculation and add in the calculation of the fixed points.   When evaluating the Sage code, we discover that there are two fixed point solutions. The first fixed point listed is . The Jacobian matrix at this fixed point is given by . Finally, Sage reports the eigenvalues as and . The dominant eigenvalue is the eigenvalue with largest magnitude . Since this eigenvalue has magnitude smaller than 1, the fixed point is stable.  The second fixed point is . The Jacobian matrix at this fixed point is given by and Sage reports the eigenvalues as and . Because the dominant eigenvalue, , has magnitude greater than 1, the fixed point is unstable.   "
},
{
  ...
  "body": " Introduction to Differential Equations   Sequences give us a way to model values at specific evenly spaced times. However, they do not provide a method to model values between those times. Furthermore, discrete time projeciton models are based on using the values of the state variables at one instant in time to predict a future value one time interval later. That is, the increment of change is considered to be a single bulk change in values.  We often want to consider models that allow us to predict values for a state at any time. We want to have the model describe how the state is changing continuously throughout time, rather than one bulk increment of change. The most basic mathematical tool to model continuous functions is a differential equation .    Rates of Change  In order to motivate differential equations, let us think back to population models where the population changed according to births and deaths. For discrete time models, we established functions that described how many births or deaths to add over the course of a cycle. Such a function represented the total accumulated births or deaths, regardless of exactly when those events occurred. For a continuous time model, we instead think in terms of the rate at which births and deaths are occurring.  A rate of change represents the ratio between the changes of two covarying variables. One of the simplest examples is velocity which is the rate of change of distance traveled with respect to time, for which you probably have heard the saying, distance equals rate times time. Mathematically, this is saying that the distance traveled is proportional to the time spent travelling and the rate (velocity) is the proportionality constant. However, this is technically only true for constant rates of change. Calculus introduced the derivative as the mathematical quantity that measures the instantaneous rate of change . This rate of change results from the idea of computing the limit of an average rate of change.  Consider a quantity that is a function of time . The instantaneous rate of change of with respect to at a particular moment is defined as the derivative . Thinking of this in reverse, this is telling us that the value can approximately be found as . That is, the increment of change in is approximately just the derivative times the increment of time. The approximation improves as the increment of time decreases and is exact in the limit.  Returning to our ideas of births and deaths, this suggests that instead of thinking of the total number of births and deaths in a given time period (e.g., per year), we will think of a rate of births or deaths per unit time so that in a short interval of time, the number of births or deaths is approximately that corresponding rate times the length of the time interval.    Differential Equations  A differential equation is an equation that defines the rate of change (a derivative) for a quantity. Unlike calculus where we would start with a function's formula and use rules of calculus to find the derivative's formula, in modeling we are going to think of the rate of change as a state variable that can be part of an equation.    We know that a population that is not constrained by its environment tends to grow exponentially. That is, we can think of the population as a function of time as satisfying an exponential model, , where and are appropriate constants. The rate of change with respect to time can be calculated using calculus rules to find .  We can identify a relation between the state variables of and by calculating the ratio and simplifying. Since is the overall rate of change and is the population, the ratio refers to the per capita rate of change . Using the exponential formula and its derivative, we discover that an exponentially growing (or decaying) population satisfies the equation . Exponential growth exactly corresponds to having a constant per capita rate of change. That is, we have a differential equation that corresponds to a population model .    Notice in the previous example that when we started with the differential equation the formula for solutions had the form . The value of is part of the differential equation. But the value of is unspecified, meaning it is free to choose. We are allowed to solve for a specific value for in order to match an initial condition.    Find a solution to the differential equation that has initial value .    Time-dependent variables (functions of time) whose derivatives are proportional to the value of the variable must be exponential. Our example has proportionality constant , so the solutions have the form .  To find the value of , we use the model formula with and use it to match the given information . From our formula, we have . Because we know (from the model formula) and (from the initial condition), we must have . That is, satisfies both the differential equation and the initial condition.    Mathematics courses focusing on solving differential equations introduce strategies for finding formulas for solutions to differential equations. Because many applications of differential equations don't actually have formula solutions, we will focus on the behavior of solutions and use computational methods to approximate those solutions.    Slope Fields  To help us understand what a differential equation does, we start by learning about a slope field . In the context of differential equations and in many scientific contexts, a field refers to a measurement that can vary as you move to different points in space. An electric field represents the potential force that would be applied to a charged particle based on its relative position to other charges in space. A gravitational field represents the potential force that would be applied to a mass based on the presence of other masses in space. A slope field indicates that at every point in space, we have an associated slope.  For differential equations, we think of our space as consisting of the time and the state. For a single state variable, say a measure of population as in our earlier example, our space will be characterized by time and the population . Consider the differential equation . The right hand side is a formula that declares a value for the slope as it depends on the current value of . Consequently, at every point in our space, we can calculate a slope given by . This is a slope field, and we would typically represent it as a function .    Given the slope field , find the slope at each of the following points.          This is not much more than evaluating the formula.  At , the slope will be .  At , the slope will be .  At , the slope will be .  At , the slope will be .      We visualize a slope field by drawing short line segments or arrows at specially chosen points, often using a grid, so that those segments have slopes matching the value of the slope field. In R using ggplot , this is accomplished with a geometry layer called geom_quiver . This requires an additional library ggquiver be installed before it can be used.  Our plot will be treated almost like a scatter plot using geom_point that specifies variables for x and y , except that our mapping must also include to additional variables u and v that will represent a displacement or rate associated with the - and -directions. Because slope graphically represents a rise over run, we will think of u as dictating a run while we think of v is dictating a rise . Using u=1 allows us to let v be the value of the slope field.  Below is a simple implementation of a slope field for the four points in the previous example. We start with a simple data frame containing the - and -values where our slopes were calculated. We will let a function perform the calculations for us.    # Load the libraries library(ggplot2) library(ggquiver) library(dplyr) # Create the initial data frame my_pts <- tibble(t = c(0, 1, 0, 1), P = c(1, -2, 3, 3)) # Define the slope field function slope <- function(t, P) { 3*P } # Start with the location of points slope_field <- my_pts |> # Then add columns for the dt and dP using slope field mutate(delta_t = 1, delta_P = slope(t, P)) |> # Then feed this into ggplot ggplot(mapping = aes(x = t, y = P, # Location u = delta_t, v = delta_P) # Directions ) + # And create the quiver graphics layer geom_quiver(color = \"blue\") show(slope_field)     Illustration of the four calculated slopes for the slope field   quiver plot showing line segments with slopes matching the slope field    It is much more informative if the slope field is visualized at a collection of points that form a regular grid in space. We will first define a grid by identifying specific and values and then construct the collection of all points from this grid. The rest of the calculations are essentially unchanged. It often takes some experimentation on the spacing chosen for the grid (see the by argument to seq ) and the scale for the arrows (see the vecsize argument to geom_quiver ) to get arrows that have nice spacing and length. Steeper arrows always have longer lengths.    # Load the libraries library(ggplot2) library(ggquiver) library(dplyr) # Create the initial grid of points my_pts <- expand.grid(t = seq(0, 2, by = 0.1), P = seq(-3.2, 3.2, by = 0.4)) # Define the slope field function slope <- function(t, P) { 3*P } # Start with the location of points slope_field_data <- my_pts |> # Then add columns for the dt and dP using slope field mutate(delta_t = 1, delta_P = slope(t, P)) # Then feed this into ggplot ggplot(data = slope_field_data, mapping = aes(x = t, y = P, # Location u = delta_t, v = delta_P) # Directions ) + # And create the quiver graphics layer geom_quiver(color = \"blue\", vecsize = 4) show(slope_field)     Illustration of the four calculated slopes for the slope field   quiver plot showing line segments with slopes matching the slope field    A slope field is related to a solution of a differential equation. If the graph of the solution, which is a function of time , is superimposed on top of a slope field, then at every point along the curve of the function, the tangent line must match the slope of the slope field. In other words, a solution follows the directions provided by the slope field, always smoothly staying parallel to the nearest segments.  The previous code listing defined slope_field as the plot showing the slope field. We can build on top of that graph additional layers that represent different solutions. We know that solutions to all have the form , so we will include several choices of functions with different values of .    # Start adding solutions as additional layers soln_curves <- slope_field + geom_function(fun = function(t) { 0.5*exp(3*t) }, linewidth=1) + geom_function(fun = function(t) { 0.01*exp(3*t) }, linewidth=1) + geom_function(fun = function(t) { -0.04*exp(3*t) }, linewidth=1) + geom_function(fun = function(t) { -1*exp(3*t) }, linewidth=1) + coord_cartesian(ylim=c(-4, 4)) # Required to keep the same viewing window show(soln_curves)     Graph showing solutions of the form superimposed on the corresponding slope field .   slope field with four solutions included      Numerical Solutions to Differential Equations  A general differential equation for a single time-dependent state variable will take the form . The function is not the derivative itself but represents the formula for the slope field. The differential equation says that the value of the rate of change, , will exactly the slope of the slope field at each point along its curve. To select a single solution, we also need to provide a starting point, called an initial condition .  The following theorem guarantees that a differential equation with an initial value has a unique solution under reasonable conditions.   Existence and Uniqueness of Initial Value Problems   Suppose that represents a continuous function of both and such that exists and is bounded (i.e., small changes in should result in small changes in . Then the differential equation with initial value has a unique solution for some interval with .    The existence property guarantees that a solution will exist on some small interval near the initial value. This gives us a justification for trying to find what that solution looks like. When we look for solutions, we do need to be mindful that we might try to use an interval that is larger than the solution can actually use. So if our method fails, we might need to try a smaller interval of time. The uniqueness property indicates that for any initial value, there can only be one solution that includes that point.  To find numerical approximations of solutions in R, we will use the deSolve library. This library defines a command ode (the letters stand for ordinary differential equation ) that will calculate data points to represent these approximate solutions. To solve our equation, we will need to define a function that calculates the function on the right-hand side of the equation. This will be very similar to the functions we used to calculate slope fields, but with some more technical requirements to deal with the variety of problems ode might encounter. The ode command also requires that we provide the initial condition and an array of times at which we will evaluate the solution.  The R function calculating the rate of change or slope required for ode is always expected to have three arguments one for the time t , one for the state state , and one for model parameters params . Both state and params are intended to allow for more than one variable, so entries are named. So that our calculations for the rate of change at the current time and state are allowed to use those internal names, we put our calculations inside of a with command. The first argument for with is a list of named values (we join state and params into a single list) that we might use in our formulas, and the second argument is a command or command block inside curly braces that does the calculation. Finally, when a state has multiple variables, we will need to report a rate of change for each variable. Consequently, ode expects that our function returns a list of rates rather than just a number. Unfortunately, I have not yet found a way to use the same function for the slope field and for ode because of these different requirements.    We will find a numerical approximation for the solution of the differential equation with the intial value and include it with the slope field generated earlier.  Our R script will need to first call library(deSolve) . Then we will define our rate function to represent the formula as described. The state will only consist of the single variable . Our model does not have any additional parameters, so the parameters will not actually be used in this example. Once the differential equation is defined, we provide the initial value and times we wish to evaluate.   R script to generate numerical solution to differential equation   # Load the library library(deSolve) # Define the rate function in this form rateFcn <- function(t, state, params) { with(as.list(c(state, params)), { Q_rate <- 3*Q # Calculate the rate list(Q_rate) # Return the rate in list }) # Close the block and end the with command } # End the function # Define the initial condition init_state <- c(Q = 0.25) # (named array of values) params <- c() # empty = no parameters # Create an array of times where we evaluate # We choose the interval of calculation times <- seq(from = 1, to = 2, by = 0.1) # Find the solution solution <- ode(init_state, times, rateFcn, params) # Result has column for time and each state variable # Create a graph with both the slope field and the solution numsoln_graph <- ggplot() + geom_quiver(data = slope_field_data, mapping = aes(x = t, y = P, u = delta_t, v = delta_P), color = \"blue\", vecsize = 4) + geom_line(data = solution, mapping = aes(x = time, y = Q), color=\"purple\", linewidth = 1) show(numsoln_graph)     Graph showing a numerical solution superimposed on the corresponding slope field .   slope field with a numerical solution       "
},
{
  "id": "intro-differential-equations-2-2",
  "level": "2",
  "url": "intro-differential-equations.html#intro-differential-equations-2-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsec-diffeq-rates-of-change-3",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-rates-of-change-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  ...
  "number": "4.1.1",
  "title": "",
  "body": "  We know that a population that is not constrained by its environment tends to grow exponentially. That is, we can think of the population as a function of time as satisfying an exponential model, , where and are appropriate constants. The rate of change with respect to time can be calculated using calculus rules to find .  We can identify a relation between the state variables of and by calculating the ratio and simplifying. Since is the overall rate of change and is the population, the ratio refers to the per capita rate of change . Using the exponential formula and its derivative, we discover that an exponentially growing (or decaying) population satisfies the equation . Exponential growth exactly corresponds to having a constant per capita rate of change. That is, we have a differential equation that corresponds to a population model .   "
},
{
  ...
  "number": "4.1.2",
  "title": "",
  "body": "  Find a solution to the differential equation that has initial value .    Time-dependent variables (functions of time) whose derivatives are proportional to the value of the variable must be exponential. Our example has proportionality constant , so the solutions have the form .  To find the value of , we use the model formula with and use it to match the given information . From our formula, we have . Because we know (from the model formula) and (from the initial condition), we must have . That is, satisfies both the differential equation and the initial condition.   "
},
{
  "id": "subsec-diffeq-slope-fields-2",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-slope-fields-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "slope field slope field "
},
{
  "id": "subsec-diffeq-slope-fields-4",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-slope-fields-4",
  "type": "Example",
  "number": "4.1.3",
  "title": "",
  "body": "  Given the slope field , find the slope at each of the following points.          This is not much more than evaluating the formula.  At , the slope will be .  At , the slope will be .  At , the slope will be .  At , the slope will be .     "
},
{
  "id": "subsec-diffeq-slope-fields-8",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-slope-fields-8",
  "type": "Listing",
  "number": "4.1.4",
  "title": "",
  "body": "  # Load the libraries library(ggplot2) library(ggquiver) library(dplyr) # Create the initial data frame my_pts <- tibble(t = c(0, 1, 0, 1), P = c(1, -2, 3, 3)) # Define the slope field function slope <- function(t, P) { 3*P } # Start with the location of points slope_field <- my_pts |> # Then add columns for the dt and dP using slope field mutate(delta_t = 1, delta_P = slope(t, P)) |> # Then feed this into ggplot ggplot(mapping = aes(x = t, y = P, # Location u = delta_t, v = delta_P) # Directions ) + # And create the quiver graphics layer geom_quiver(color = \"blue\") show(slope_field)   "
},
{
  "id": "fig-diffeq-first-slopefield",
  "level": "2",
  "url": "intro-differential-equations.html#fig-diffeq-first-slopefield",
  "type": "Figure",
  "number": "4.1.5",
  "title": "",
  "body": " Illustration of the four calculated slopes for the slope field   quiver plot showing line segments with slopes matching the slope field   "
},
{
  "id": "subsec-diffeq-slope-fields-11",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-slope-fields-11",
  "type": "Listing",
  "number": "4.1.6",
  "title": "",
  "body": "  # Load the libraries library(ggplot2) library(ggquiver) library(dplyr) # Create the initial grid of points my_pts <- expand.grid(t = seq(0, 2, by = 0.1), P = seq(-3.2, 3.2, by = 0.4)) # Define the slope field function slope <- function(t, P) { 3*P } # Start with the location of points slope_field_data <- my_pts |> # Then add columns for the dt and dP using slope field mutate(delta_t = 1, delta_P = slope(t, P)) # Then feed this into ggplot ggplot(data = slope_field_data, mapping = aes(x = t, y = P, # Location u = delta_t, v = delta_P) # Directions ) + # And create the quiver graphics layer geom_quiver(color = \"blue\", vecsize = 4) show(slope_field)   "
},
{
  "id": "fig-diffeq-slopefield-grid",
  "level": "2",
  "url": "intro-differential-equations.html#fig-diffeq-slopefield-grid",
  "type": "Figure",
  "number": "4.1.7",
  "title": "",
  "body": " Illustration of the four calculated slopes for the slope field   quiver plot showing line segments with slopes matching the slope field   "
...
{
  "id": "subsec-diffeq-slope-fields-15",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-slope-fields-15",
  "type": "Listing",
  "number": "4.1.8",
  "title": "",
  "body": "  # Start adding solutions as additional layers soln_curves <- slope_field + geom_function(fun = function(t) { 0.5*exp(3*t) }, linewidth=1) + geom_function(fun = function(t) { 0.01*exp(3*t) }, linewidth=1) + geom_function(fun = function(t) { -0.04*exp(3*t) }, linewidth=1) + geom_function(fun = function(t) { -1*exp(3*t) }, linewidth=1) + coord_cartesian(ylim=c(-4, 4)) # Required to keep the same viewing window show(soln_curves)   "
},
{
  "id": "fig-diffeq-slopefield-solutions",
  "level": "2",
  "url": "intro-differential-equations.html#fig-diffeq-slopefield-solutions",
  "type": "Figure",
  "number": "4.1.9",
  "title": "",
  "body": " Graph showing solutions of the form superimposed on the corresponding slope field .   slope field with four solutions included   "
},
{
  "id": "subsec-diffeq-numerical-solutions-2",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-numerical-solutions-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
},
{
  ...
  "body": " Existence and Uniqueness of Initial Value Problems   Suppose that represents a continuous function of both and such that exists and is bounded (i.e., small changes in should result in small changes in . Then the differential equation with initial value has a unique solution for some interval with .   "
},
{
  "id": "subsec-diffeq-numerical-solutions-8",
  "level": "2",
  "url": "intro-differential-equations.html#subsec-diffeq-numerical-solutions-8",
  "type": "Example",
  "number": "4.1.11",
  "title": "",
...
{
  "id": "sec-diffeq-equilibria",
  "level": "1",
  "url": "sec-diffeq-equilibria.html",
  "type": "Section",
  "number": "4.3",
  "title": "Analysis of Equilibria, Drug Dosing, and the Allee effect",
  "body": " Analysis of Equilibria, Drug Dosing, and the Allee effect   The logistic differential equation model provided an example where the model had two equilibria, one of which was stable and the other was unstable. When a differential equation is autonomous so that , then the equilibria correspond to the roots of solving . In this section, we will focus on some core principles about analyzing a differential equation including the stability of equilibria.    The Phase Line  In a differential equation, the state variable is a function of time. Suppose that is our state variable in a differential equation . Finding a solution means that we are finding a function  so that at each point on the graph of the function has a slope that is equal to the slope field formula . The graph of the function lives in a 2-dimensional space of points .  We want to think of as existing in a 1-dimensional space representing the state alone , but where the state changes for different values of time . I like to think of as if it is a point living on a line, which we call the phase line . As time advances, that point moves around on the line. The function represents how the point's location depends on time.  The figure below illustrates this idea with a video. Time is represented on the horizontal axis. The phase line is illustrated as a line overlapping the vertical axis for our state variable . In the video, as time advances, the point on the phase line moves to match the value of on the graph.   moves along the phase line as a function of time .    Now, consider possible behaviors of solutions for a time autonomous differential equation, . Because the differential equation does not itself depend on time, the direction of motion only depends on the value of . From calculus, we know that when , the function is increasing , and when , the function is decreasing . Consequently, if we look at a graph of the slope field function as a function in its own right, we can use the signs of to determine direction of motion on the phase line.    Let be a slope field function that is independent of time and suppose that is continuous .  Points where are equilibria. Solutions that start at this point stay there forever.  If starts at a point in an interval where , then is increasing and moves in the positive direction within that interval.  If starts at a point in an interval where , then is decreasing and moves in the negative direction within that interval.      As a consequence of the existence and uniqueness theorem, a solution can not move through an equilibrium (which is itself a solution). Consequently, the roots in and intervals associated with completely characterize the directions of solutions.    The logistic differential equation has as its slope field function . This is a quadratic function that opens down and has roots at and .   Graph of rate function for the logistic model   The phase line will have equilibrium points at and at . On the interval , the rate function is positive so will be increasing on that interval. On the interval , the rate function is negative so will be decreasing on that interval. Similarly, is decreasing on .   Phase line diagram associated with the logistic model     We can use the phase line diagram to characterize the stability of an equilibrium. If the phase line diagram shows solutions on both sides moving toward the equilibrium, then that equilibrium is stable. If either side of the phase line has solutions that move away from the equilibrium, then the equilibrium is unstable.  The behavior of the phase line near the equilibrium can also be immediately determined by looking at the derivative of the rate function at the equilibrium. Notice that the slope of the logistic rate function is positive at . This necessarily means that is negative on the left and is positive on the right. In general, any equilibrium where will be unstable . On the other hand, the equilibrium at has a negative slope. The values of near that point are positive on the left and negative on the right. In general, any equilibrium where will be stable .   Derivative Test for Stability   Suppose a differential equation has an equilibrium at , with and suppose exists.  If , then is an unstable equilibrium.  If , then is a stable equilibrium.  For the case where , the test is inconclusive. (Check the phase line directly.)      Applications  Applying the principles of interpreting the phase line for a differential equation is an example of qualitative analysis . We have not solved the differential equation but we still understand the basic behavior that it will create.  The body metabolizes many drugs so that the drug is cleared from the bloodstream at a rate that is proportional to the drug concentration. Suppose we are considering the blood concentration of a patient in a hospital that is being dosed using an IV. This can be modeled as adding drug into the blood stream at a constant rate. What would a differential equation predict about the patient's drug concentration?  We start by creating a differential equation. The state variable is the amount of drug in the bloodstream, . The IV drip corresponds to a source of adding drug into the system. This occurs at a constant rate, say at a rate . The body metabolizes the drug, removing it from the blood stream, at a rate that is proportional to the amount of drug. If we use a positive proportionality constant , then the drug is being cleared at a rate . The overall rate of change is the rate of drug being added minus the rate of drug being removed, leading to our differential equation .  The rate function defining our slope field is linear in , , where is the -intercept and is the slope. There is an equilibrium where , which occurs at . We know that , which is negative, so is a stable equilibrium. If the patient starts with , the drug concentration will rise steadily and then converge .   Vancomycin is an antibiotic that is commonly administered intravenously. The typical recommended intravenous dosing is 500 mg per hour (for an hour). The data sheet for the drug lists a half-life of approximately 5 hours. If the dosing were to continue at this rate indefinitely, what would be the equilibrium? How close do we get to equilibrium after the one hour of dosing?  To apply this information to the model, we have the constant rate of dosage as (mg\/hr). The half-life allows us to find the rate constant . In the absence of additional doses of drugs, our model for the drug in the blood is just , which has an exponential solution . The half-life is the time it takes to go from to half that value . Using the model, we have , which allows us to solve for . With (hr), this means .  We now have our differential equation, with an initial value (no drug at the start of treatment). The equilibrium can be found by solving to obtain (mg). At the end of only one hour, we have only administered 500 mg, but the body will have already metabolized some of that. To find the amount in the body, we need to solve the differential equation with the given initial value. In our graph, we will compare the amount in the bloodstream with the total amount administered to that point.    # Model parameters model_params <- c(R_IV = 500, k = -log(0.5)\/5) # Define the rate function rateF <- function(t, state, params) { with(as.list(c(state, params)), { D_rate <- R_IV - k * D list(D_rate) }) } # Solve the initial value problem library(deSolve) times <- seq(0, 1, by=0.05) init_state <- c(D = 0) soln <- ode(init_state, times, rateF, model_params) # Show a graph library(ggplot2) soln_graph <- ggplot(data = soln, mapping = aes(x = time, y = D)) + geom_abline(slope = 500, color=\"red\", linetype = \"dashed\") + geom_line(color=\"blue\", linewidth = 1) + labs(y = \"Vancomycin in blood [mg]\") show(soln_graph) # Display the final row of data N <- length(times) print(soln[N, ])     Amount of vancomycin in bloodstream for an administration rate of 500 mg per hour over the course of one hour.   graph of vancomycin in bloodstream over 1 hour    Examining the final value in the solution, we learn that after one hour, the amount of drug in the bloodstream is 466.89 mg, while 500 mg have been administered. The body has already metabolized 33.11 mg during that time. If we were to extend our simulation further in time, say a full day, we will see the blood level leveling out near the equilibrium value. Even after that much time, the amount of the drug in the bloodstream is 3477 mg, nearly 130 mg away from the equilbrium.    # Use the same as before but with more time. times <- seq(0, 24, by=0.25) init_state <- c(D = 0) soln <- ode(init_state, times, rateF, model_params) # Show a graph library(ggplot2) soln_graph <- ggplot(data = soln, mapping = aes(x = time, y = D)) + geom_line(color=\"blue\", linewidth = 1) + labs(y = \"Vancomycin in blood [mg]\") show(soln_graph) # Display the final row of data N <- length(times) print(soln[N, ])     Amount of vancomycin in bloodstream for an administration rate of 500 mg per hour over the course of one full day.   graph of vancomycin in bloodstream over 24 hours     When we looked at density dependence for a single population, we developed the logistic model by considering the per capita growth rate as being a decreasing linear function. The biological argument here was that for larger population densities, there is more competition or other negative impacts that result in either falling birth rates or rising death rates. However, some species actually exhibit the per capita growth rates increase over a range of low population values before ultimately decreasing for very large population values. This behavior is called the Allee effect , named after Warder Allee who studied increased survival for goldfish in the presence of higher densities of other goldfish.   Per capita growth rate as a function of the population size. Left: logistic growth uses a linear decreasing function. Right: Allee effect shows a switch between increasing and decreasing as a function.   side by side view of per capita growth for the logistic model with that for the Allee effect model    The simplest model for the per capita growth rate that captures the behavior of the Allee effect would be a negative quadratic function. The vertex of the quadratic should have a positive value, so there will be two roots where that quadratic function crosses the axis. The larger of the two will play the role of the carrying capacity, so we will name it . The lower root will be . We can represent the quadratic in a factored form, for some positive parameter . With only natural growth and losses, the resulting differential equation will become , which will take the form .  Our Allee effect model will mathematically have three equilibria , and . The behavior will depend on whether we have or . A negative cubic with three roots has a graph that has the basic form shown in .   Typical graph of the total growth rate for cubic Allee effect model with three roots.   graph of a negative cubic with three real roots    If , our model has two positive equilibria and in addition to the equilibrium that corresponds to extinction. The graph makes it clear in that in this case that at the leftmost equilibrium , . For initial values , the growth rate is negative and the population will decrease toward extinction. If the initial value starts above , the solution will move toward the carrying capacity . This case is called the strong Allee effect , in that there is a range of small positive population values for which decreases ( ). The value is a critical threshold that a population must stay above in order to grow and ultimately reach its capacity. If the population falls below this Allee threshold, the population will be driven toward extinction.   On the other hand, if , our model has only one positive equilibrium to go with the extinction equilibrium . This time, starting with a small population still results in , and the population will grow. Every initial positive population will result in a solution that converges to the carrying capacity. This case is called the weak Allee effect .   I have not yet discovered definitive data that allows estimating actual Allee effect model parameters. Research articles focus mainly on evidence of increased survival rates or reproductive success, but don't directly measure how that impacts overall population growth rates. In the following example, we will show how dimensional analysis allows us to realize general possible behaviors without needing detailed rates.   The Allee effect model for population growth using a quadratic per capita growth rate results in the cubic rate function, . We will consider the strong Allee effect so that . The carrying capacity can provide a natural scale for the population, so that we write , where is the dimensionless size of the population relative to . We also need a scale for time that is not yet clear, so we write where will be our unit of time and will be the dimensionless measure of time.  We next rewrite the differential equation in terms of and . The rate of change will have units , so that we can rewrite the original equation as . Factoring out from the two terms and , and then multiplying both sides of the equation by , we arrive at a nondimensionalized differential equation . We still get to choose our value of , and if we choose , the leading coefficient becomes 1. The only remaining dependence on our original parameters is the ratio , which measures the size of the Allee threshold in units of .  With these choices, the Allee effect differential equation becomes . If we explore this differential equation with different values of , we will understand all of the possible behaviors. Different choices for , , and will have solutions that correspond to rescaled solutions of the solutions that we find.  First, let us consider the Allee effect with , putting the threshold at 20% of the carrying capacity. The following script illustrates finding numerical solutions for different initial conditions and displaying them on top of a slope field. Other choices of or initial conditions can be achieved by modifying the appropriate lines in the script.    # Model parameters model_params <- c(m = 0.2) # List of initial values (will be in a loop) initial_values = c(0, 0.15, 0.20, 0.25, 0.5, 1, 1.5) # Define the rate function slopefield_fcn <- function(p) { with(as.list(model_params), -p*(p-m)*(p-1)) } # Create the slope field graph library(dplyr) library(ggplot2) library(ggquiver) grid_pts <- expand.grid(t = seq(0, 20, by=1), p = seq(-0.2, 1.25, by=0.05)) # Calculate the slopes slope_field_data <- mutate(grid_pts, delta_t = 1, delta_p = slopefield_fcn(p) ) slope_field_graph <- ggplot() + geom_quiver(data = slope_field_data, mapping = aes(x = t, y = p, u = delta_t, v = delta_p), color = \"blue\", vecsize = 5) show(slope_field_graph) # Prepare to solve the differential equation library(deSolve) rateF <- function(t, state, params) { with(as.list(c(state, params)), { p_rate <- -p*(p-m)*(p-1) list(p_rate) }) } # We are going to add multiple solutions in a loop. # Start with the slope field as the base layer soln_plot <- slope_field_graph + coord_cartesian(ylim=c(-0.2, 1.25)) + labs(x = expression(tau)) # Loop over initial values for (p0 in initial_values) { # Solve the initial value problem times <- seq(0, 20, by=0.25) init_state <- c(p = p0) soln <- ode(init_state, times, rateF, model_params) # Add the graphic layer for this solution soln_plot <- soln_plot + geom_line(data = soln, mapping = aes(x = time, y = p), color = \"purple\", linewidth = 1) } # Show the result show(soln_plot)     Solutions for the Allee effect differential equation with the strong Allee effect, .   solution curves for the strong Allee effect      "
},
{
  "id": "subsec-diffeq-phase-line-3",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-diffeq-phase-line-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "phase line "
...
{
  "id": "fig-movie-phase-line-point-moving",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#fig-movie-phase-line-point-moving",
  "type": "Figure",
  "number": "4.3.1",
  "title": "",
  "body": " moves along the phase line as a function of time .   "
},
{
  "id": "subsec-diffeq-phase-line-6",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-diffeq-phase-line-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "thm-phase-line-directions",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#thm-phase-line-directions",
  "type": "Theorem",
  "number": "4.3.2",
  "title": "",
  "body": "  Let be a slope field function that is independent of time and suppose that is continuous .  Points where are equilibria. Solutions that start at this point stay there forever.  If starts at a point in an interval where , then is increasing and moves in the positive direction within that interval.  If starts at a point in an interval where , then is decreasing and moves in the negative direction within that interval.     "
},
{
  "id": "subsec-diffeq-phase-line-9",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-diffeq-phase-line-9",
  "type": "Example",
  "number": "4.3.3",
  "title": "",
  "body": "  The logistic differential equation has as its slope field function . This is a quadratic function that opens down and has roots at and .   Graph of rate function for the logistic model   The phase line will have equilibrium points at and at . On the interval , the rate function is positive so will be increasing on that interval. On the interval , the rate function is negative so will be decreasing on that interval. Similarly, is decreasing on .   Phase line diagram associated with the logistic model    "
},
{
  "id": "subsec-diffeq-phase-line-11",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-diffeq-phase-line-11",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "unstable stable "
},
{
  "id": "thm-diffeq-stability-derivative-test",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#thm-diffeq-stability-derivative-test",
  "type": "Theorem",
  "number": "4.3.4",
  "title": "Derivative Test for Stability.",
  "body": " Derivative Test for Stability   Suppose a differential equation has an equilibrium at , with and suppose exists.  If , then is an unstable equilibrium.  If , then is a stable equilibrium.  For the case where , the test is inconclusive. (Check the phase line directly.)   "
},
{
  "id": "subsec-equilibria-examples-2",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-equilibria-examples-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "qualitative analysis "
},
{
  "id": "subsec-equilibria-examples-5",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-equilibria-examples-5",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "stable "
...
{
  "id": "subsec-equilibria-examples-6",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-equilibria-examples-6",
  "type": "Example",
  "number": "4.3.5",
  "title": "",
  "body": " Vancomycin is an antibiotic that is commonly administered intravenously. The typical recommended intravenous dosing is 500 mg per hour (for an hour). The data sheet for the drug lists a half-life of approximately 5 hours. If the dosing were to continue at this rate indefinitely, what would be the equilibrium? How close do we get to equilibrium after the one hour of dosing?  To apply this information to the model, we have the constant rate of dosage as (mg\/hr). The half-life allows us to find the rate constant . In the absence of additional doses of drugs, our model for the drug in the blood is just , which has an exponential solution . The half-life is the time it takes to go from to half that value . Using the model, we have , which allows us to solve for . With (hr), this means .  We now have our differential equation, with an initial value (no drug at the start of treatment). The equilibrium can be found by solving to obtain (mg). At the end of only one hour, we have only administered 500 mg, but the body will have already metabolized some of that. To find the amount in the body, we need to solve the differential equation with the given initial value. In our graph, we will compare the amount in the bloodstream with the total amount administered to that point.    # Model parameters model_params <- c(R_IV = 500, k = -log(0.5)\/5) # Define the rate function rateF <- function(t, state, params) { with(as.list(c(state, params)), { D_rate <- R_IV - k * D list(D_rate) }) } # Solve the initial value problem library(deSolve) times <- seq(0, 1, by=0.05) init_state <- c(D = 0) soln <- ode(init_state, times, rateF, model_params) # Show a graph library(ggplot2) soln_graph <- ggplot(data = soln, mapping = aes(x = time, y = D)) + geom_abline(slope = 500, color=\"red\", linetype = \"dashed\") + geom_line(color=\"blue\", linewidth = 1) + labs(y = \"Vancomycin in blood [mg]\") show(soln_graph) # Display the final row of data N <- length(times) print(soln[N, ])     Amount of vancomycin in bloodstream for an administration rate of 500 mg per hour over the course of one hour.   graph of vancomycin in bloodstream over 1 hour    Examining the final value in the solution, we learn that after one hour, the amount of drug in the bloodstream is 466.89 mg, while 500 mg have been administered. The body has already metabolized 33.11 mg during that time. If we were to extend our simulation further in time, say a full day, we will see the blood level leveling out near the equilibrium value. Even after that much time, the amount of the drug in the bloodstream is 3477 mg, nearly 130 mg away from the equilbrium.    # Use the same as before but with more time. times <- seq(0, 24, by=0.25) init_state <- c(D = 0) soln <- ode(init_state, times, rateF, model_params) # Show a graph library(ggplot2) soln_graph <- ggplot(data = soln, mapping = aes(x = time, y = D)) + geom_line(color=\"blue\", linewidth = 1) + labs(y = \"Vancomycin in blood [mg]\") show(soln_graph) # Display the final row of data N <- length(times) print(soln[N, ])     Amount of vancomycin in bloodstream for an administration rate of 500 mg per hour over the course of one full day.   graph of vancomycin in bloodstream over 24 hours    "
},
{
  "id": "subsec-equilibria-examples-7",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-equilibria-examples-7",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Allee effect "
...
{
  "id": "fig-per-capita-growth-rate-Allee",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#fig-per-capita-growth-rate-Allee",
  "type": "Figure",
  "number": "4.3.10",
  "title": "",
  "body": " Per capita growth rate as a function of the population size. Left: logistic growth uses a linear decreasing function. Right: Allee effect shows a switch between increasing and decreasing as a function.   side by side view of per capita growth for the logistic model with that for the Allee effect model   "
},
{
  "id": "fig-total-growth-rate-Allee",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#fig-total-growth-rate-Allee",
  "type": "Figure",
  "number": "4.3.11",
  "title": "",
  "body": " Typical graph of the total growth rate for cubic Allee effect model with three roots.   graph of a negative cubic with three real roots   "
},
{
  "id": "subsec-equilibria-examples-12",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-equilibria-examples-12",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "strong Allee effect threshold "
},
{
  "id": "subsec-equilibria-examples-14",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-equilibria-examples-14",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "weak Allee effect "
},
{
  "id": "subsec-equilibria-examples-17",
  "level": "2",
  "url": "sec-diffeq-equilibria.html#subsec-equilibria-examples-17",
  "type": "Example",
  "number": "4.3.12",
  "title": "",
...
{
  "id": "subsec-diffeq-systems-differential-equations-28",
  "level": "2",
  "url": "seq-diffeq-interacting-populations.html#subsec-diffeq-systems-differential-equations-28",
  "type": "Figure",
  "number": "4.4.13",
  "title": "",
  "body": " Interactive dynamics of the producer-consumer model   "
},
{
  "id": "seq-diffeq-nullclines-predation-competition",
  "level": "1",
  "url": "seq-diffeq-nullclines-predation-competition.html",
  "type": "Section",
  "number": "4.5",
  "title": "Phase Plane Analysis with Nullclines, Competition and Predator-Prey Models",
  "body": " Phase Plane Analysis with Nullclines, Competition and Predator-Prey Models   For a single autonomous differential equation , solving the equation allowed us to identify the equilibrium solutions. When we have an autonomous system of two differential equation, solving a single equation like or is not quite finding an equilibrium. An equilibrium would require that both equations are satisfied for the same point. So what do we get when we think of an individual equation at a time?  In the phase plane, an equation of the form generally describes a curve in the plane. In calculus, you would have learned about something called an implicit function . In a general sense, for each value of , there can be zero, one, or more values of that make the equation true. Near most such points, the solutions will follow some curve and the slope of that curve is calculated using the calculus strategy of implicit differentiation .  While we don't need to focus heavily on the slope of the curve, we are interested in the idea that the equation describes a curve in the plane. Because the equation represents points where would have zero rate of change, the curve is called the - nullcline . The curve satisfied by is called the -nullcline. Any equilibrium solutions will correspond to the intersections of these two nullclines.  In this section, we will discuss how finding or describing the nullclines can simplify the visualization of the phase plane. We can use intersection points to find equilibria. Instead of finding a full vector field, we can identify direction at a single point in each region formed by the nullclines to see general directions of motion. Sometimes, if it is clear that either all directions point toward the equilibrium or at least one direction points away, this is enough to determine the stability of equilibrium values.  As our primary examples to motivate the analysis, we will discuss competition between two species. We will also return to the idea of predation and consider predator-prey models.    Competition Models  The Rusty crayfish is an invasive aquatic crustacean in Virginia. (See .) Native in Ohio, Kentucky, Tennessee, and Indiana, this crayfish was first reported in Virginia in 2011. The species competes for resources with native crayfish but consume resources at a much higher rate. Thus, the Rusty crayfish risks displacing native species but also has the potential to disrupt the aquatic environment of other native species who share the water.  Invasive species are often studied in the context of competition models . We want to consider two species that are found in the same environment and compete for similar resources, such as populations of native crayfish and Rusty crayfish. If there were only one species present, we could model that species with a logistic differential equation. What does the presence of a competitor do to the model?  If we imagined that a carrying capacity represents the available number of slots available for a species in the environment based on resource availability, then the presence of a competitor who uses some of those resources is effectively eliminating some of the available slots. From the perspective of per capita growth rates, we can imagine that the presence of a competitor has a negative effect on the species per capita rate.  Let represent the population size of native crayfish and let represent the population size of Rusty crayfish. Each population would have its own intrinsic per capita growth rates and carrying capacities for the given environment, say and for the native population, and and for the invasive population. The presence of competitors occupy some of the resources of the other species, so we can model our two populations using modified logistic growth models, The parameter represents the (fractional) number of native crayfish that would have the same impact on growth rates of native crayfish as each Rusty crayfish has. Similarly, the parameter represents the (fractional) number of Rusty crayfish that would have an equivalent impact on Rusty crawfish growth rates compared to the presence of each native crayfish.    Nullcline Analysis  A nullcline is a curve defining a collection of points in the phase plane at which one of the state variables has an instantaneous rate of change equal to zero. We need to emphasize the idea of instantaneous because other variables are likely changing so that the solutions immediately move off of the nullcline.  For our competition model, the -nullcline (where has a zero rate of change) is characterized by points at which Similarly, the -nullcline is characterized by points at which Because each equation is already written in a factored form, each nullcline consists of two distinct curves. The -nullcline consists of the line for which as well as a second line for which . The -nullcline consists of the lines for which or .  Just as we drew arrows on a phase line to see the direction of motion for a single state variable, we can use nullclines to decompose the phase plane into different regions where each state variable is increasing or decreasing. In the phase plane, the -nullcline includes a vertical line and another line that has an -intercept ( ) at and an intercept at . We check the sign of the rate formula to determine if is increasing or decreasing. When is larger than the second line, we have  decreasing . When is smaller than the second line, we have  increasing . Having is not biological for populations, but mathematically, we would have a reversal of signs because of the factor being negative.  We have a similar evaluation for the variable . The nullcline has a horizontal line as well as a line with slope . That second line has an -intercept at and a -intercept at . Regions of where is increasing or decreasing is analogous to that of .  Both nullclines are visualized in along with directions of motion for and . The behaviors are shown in separate graphs because we still need to consider the relative position of the nullclines to each other.   (left) and (right) nullclines as well as regions of the phase plane marked by whether the state variable is increasing or decreasing.    N-nullcline and regions where N is increasing or decreasing    R-nullcline and regions where R is increasing or decreasing     In order to analyze the dynamics, we consider the ordering of the - and -intercepts. For -intercepts, we either have or . (There is also an exceptional case where they are equal, but the analysis for this would distract from our basic understanding.) Similarly, the -intercepts have two possible orderings. Considering all of the combinations, there are four basic orderings possible, shown in .   All possible orderings of the intercepts and resulting relative positions for the - and -nullclines for the competition model.    N-nullcline intercepts are both above the R-nullcline intercepts    nullclines cross with equilibria intercepts above the non-equilibria intercepts      nullclines cross with non-equilibria intercepts above the equilibria intercepts    R-nullcline intercepts are both above the N-nullcline intercepts     In each region of , we have included a single pair of direction arrows showing whether and are increasing or decreasing, with a composite direction formed by the addition of those vectors. Notice how the -direction reverses when we cross the -nullcline. This is analogous to ordinary functions changing sign when the variable passes through a root. (Caution: A function can in general have a root without changing sign, like a parabola touching the axis at a single point; the same could happen with nullclines.)  Points where the -nullcline intersects the -nullcline are equilibrium points. There are three equilibria that appear in every situation. The equilibrium corresponds to mutual extinction, where neither population is present. The equilibrium corresponds to the situation where the native crayfish are at their carrying capacity and there are no Rusty crayfish present. Conversely, the equilibrium corresponds to the situation where the Rusty crayfish are at their carrying capacity having completely wiped out the native population. There are two cases where the nullclines intersect in the region with and . It is possible to solve for the exact location of that point, but we will just call it .  The directions of the vectors help us to recognize which equilibria are stable and which are stable. If the direction arrows point toward an equilibrium in every region that surrounds the equilibrium, it must be stable. If there is at least one region for which it is clear that direction arrows point away from the equilibrium, it must be unstable. There can also be a case (not illustrated in this example) where it looks like direction arrows look like they would go around an equilibrium. In that case, we would require another analysis method (involving eigenvalues) to determine stability.  In all cases, the equilibrium is unstable . For points near the equilibrium but with and , we see that both and will increase, forcing solutions to move away from the equilibrium. Biologically, this means that an environment with no crayfish of either species for which we introduce new crayfish, those crayfish populations will (at least initially) grow.  Next, let us consider the equilibrium . In , the top two cases and bottom two cases show different behaviors, based on the ordering of the -intercepts. In the top cases, where (the equilibrium) is greater than (the -intercept of the -nullcline), the regions around the equilibrium have all direction arrows pointing toward the equilibrium. Consequently, in these cases the equilibrium would be stable .  Biologically, the equilibrium represents the pre-invasion state of the ecosystem. The native population has reached its own carrying capacity and is at equilibrium. Stability means that when the new species is introduced with a few individuals, corresponding to moving to a point in the phase plane just above the equilibrium, the population will be driven back toward just one population. The invading species would disappear and the invasion attempt would be unsuccessful.  In the other two cases, where is less than , the regions around the equilibrium show the directions point toward the equilibrium but the directions point away from the equilibrium. In these cases, the equilibrium is unstable . Biologically, this would mean that when a few Rusty crayfish enter a native population of crayfish at equilibrium, the Rusty crayfish population is able to grow, moving the system away from equilbrium. This would be a successful invasion.  The equilibrium will be essentially analogous to the previous discussion, except with the roles of and reversed. Biologically, this refers to an ecosystem that only contains the Rusty crayfish. In the context of the invasion, since we had to start with just the native population, this would conceptually be an equilibrium corresponding to a complete takeover by the invader. Being a stable equilibrium would mean that reintroducing a few native crayfish without eliminating the Rusty crayfish would have no impact.  There is a fourth equilibrium in the positive quadrant in two of the four cases shown in . Notice that this extra equilibrium has stability that is opposite that of and . In the top-right case where and , both of the single population equilibria are stable, but the interior equilibrium is unstable because solutions in the narrow wedges have directions moving away from the equilibrium. This case corresponds to a scenario in which a single population is resistant to invasion by the other, regardless of which population happened to have initially colonized the area.  In the bottom-left case, the two single population equilibria are both unstable, but the interior equilibrium is stable. This corresponds to a scenario where both types of crayfish are susceptbile to an invasion, but the invasion results in a stable equilibrium where both species coexist. This would typically only happen for two species that have weak competition, where the presence of one slows the other, but there are enough resources needs that do not overlap that both species can be self-sustaining in the presence of the other.    The Predator-Prey Model  We previously considered a producer-consumer model between two species. We will revisit a simplified version of that model called the Lotka-Volterra Predator Prey model . We think of the producer species as any prey species, and the consumer species is the predator species. For example, we might consider a prey species as being field mice and the predator species as being owls.  The model will be simplified in that the prey species does not have logistic growth but is modeled with a constant per capita growth rate. If we let represent the mouse population size and represent the owl population size, then the Lotka-Volterra model would be written as the system of differential equations   Factoring the equations will make the nullclines more apparent, so we rewrite our model as The nullcline in the phase plane will be found from or , corresponding to the vertical line and the horizontal line . Similarly, the nullcline will consist of the horizontal line and the vertical line .  Directions for the rates of change in the positive quadrant can be characterized by looking at the sign of the rate formulas on either side of the nullclines.  For the mice , if (many owls), the rate of change in the mouse population will be negative. Owls are eating mice faster than they are able to reproduce because there are so many owls. However, when , the rate of reproduction now exceeds predation by owls and the mouse population is able to grow.  Similarly, for the owls, if (many mice), the rate of change for the owl population will be positive. There is plenty of food for the owls and the owl population will be increasing. On the other hand, if (few mice), the rate of change for the owl population will be negative and the owl population will decrease.  The phase plane with nullclines and directions of motion are illustrated in . Notice that there are exactly two equilibria, where represents mutual extinction and represents both species coexisting.   Nullclines in the phase space for the owl-mice predator-prey model.   N-nullcline intercepts are both above the R-nullcline intercepts    The vector field directions this time are only partly helpful in understanding stability. The equilibrium for extinction is clearly unstable based on directions of the vector field, since a small introduction of mice into the environment will result in mice growing exponentially. However, the vector field does not show clearly what happens for the coexisting equilibrium the vector field seems to rotate around the equilibrium point, rather than flow in or out of it. We need deeper analysis than just the picture to determine what happens here.    Generating Nullcline Curves in R  The discussion earlier was algebra-centered. Using nullclines is often a qualitative method that we use in a preliminary setting and may not be included in our final analysis. Sometimes, we our nullclines can be easily visualized in terms of graphs that we know, such as the lines that appeared in our earlier examples. At other times, we might not see how to express the nullclines in a way that makes it easy for us to graph.  One option would be to use a graphing tool like desmos.com where you can type an equation, although you need to use and as your state variables. An advantage here is that if you use parameter names, you get automatic sliders to adjust any of those parameters and can explore the effect of changing them. Other tools might also be able to do this, often using a command to create implicit plots .  This section introduces some ideas for how we can visualize the results in R using contour plots. We might want to do this so that everything is integrated together into a single document. It would also allow us to overlay solution trajectories on a graph that shows the nullclines if desired.  When we calculate a vector field at a grid of points, we gain some a lot of information. Nullclines are significant because they show where one of the directions rate functions goes to zero. Usually, the vector field changes direction for a state variable when we cross that state's nullcline. This would be a change of sign in the data calculated for our vector field.  A contour graph takes a grid of two variables and a function or data that is evaluated at each point in the grid and shows curves where the value of the function or data changes between various levels. To find a nullcline, we want to find a curve where the data crosses the value of zero, thereby separating the data into two regions for positive and negative.  If we know how to create a vector field for a differential equation, we can use the same setup to estimate the nullclines. However, we often want a more closely-spaced grid of points for a nullcline in order to make the estimate smooth, so we probably don't want to use the exact same grid data.    For our first example, let us generate an estimate of the nullclines for the competition model. We will use state variables and for the system of differential equations with so that our carrying capacities are and , respectively. We will choose and , and and .    # Define our parameters my_p <- c(r1 = 0.2, r2 = 0.3, alpha = 0.75, beta = 1.1) # Define our rate functions f1 <- function(X, Y, params) { with(as.list(params), r1 * X * (1 - X - alpha * Y) ) } f2 <- function(X, Y, params) { with(as.list(params), r2 * Y * (1 - Y - beta * X) ) } # Make a vector field grid (relatively wide spacing) library(dplyr) # To get mutate (add extra columns) vf_grid <- expand.grid(X = seq(-0.5, 1.5, by = 0.2), Y = seq(-0.5, 1.5, by = 0.2)) |> mutate(dXdt = f1(X, Y, my_p), dYdt = f2(X, Y, my_p)) |> # rescale by speed mutate(speed = sqrt(dXdt^2 + dYdt^2)) |> mutate(Xdir = dXdt \/ speed, Ydir = dYdt \/ speed) # Make a contour grid (relatively close spacing) contour_grid <- expand.grid(X = seq(-0.5, 1.5, by = 0.02), Y = seq(-0.5, 1.5, by = 0.02)) |> mutate(dXdt = f1(X, Y, my_p), dYdt = f2(X, Y, my_p)) # Make a graph showing the vector field and nullclines library(ggplot2) library(ggquiver) nullcline_graph <- ggplot() + # Vector field geom_quiver(data = vf_grid, mapping = aes(x = X, y = Y, u = Xdir, v = Ydir), vecsize = 0.25) + # X nullcline geom_contour(data = contour_grid, mapping = aes(x = X, y = Y, z = dXdt, color = \"X\"), breaks = c(0), linetype = \"dashed\") + # Y nullcline geom_contour(data = contour_grid, mapping = aes(x = X, y = Y, z = dYdt, color = \"Y\"), breaks = c(0), linetype = \"dashed\") + # Legend update scale_color_discrete(breaks = c(\"X\",\"Y\"), name = \"nullcline\") show(nullcline_graph)     Nullclines and vector field associated with the competition model given in the   generated graph showing nullclines and vector field described in listing      A disadvantage of using contours from data as opposed to an algebraic approach is that we have to choose specific model parameters. An algebraic approach can allow us to recognize how the nullclines will change as we adjust various parameters.  A second issue is that we only get an approximation, and sometimes the result is not quite accurate. For example, the nullcline that was created for the competition model in the example above used by = 0.02 for the grid used by the contour command. If we changed that to by = 0.06 , we would see a result where there appears to be a gap in the nullclines. What is happening is that at the point where the nullcline intersects itself, unless we happen to include that point in the grid, the algorithm doesn't know the point exists and allows two regions on opposite sides of the point to merge to a single region.   Recreated with by=0.06 , which results in a gap in the nullclines   remade graph showing nullclines and vector field described in listing with different step size     "
},
{
  "id": "seq-diffeq-nullclines-predation-competition-2-3",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#seq-diffeq-nullclines-predation-competition-2-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "nullcline "
...
{
  "id": "subsec-diffeq-competition-models-3",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#subsec-diffeq-competition-models-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "competition models "
},
{
  "id": "fig-competition-nullclines-directions",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#fig-competition-nullclines-directions",
  "type": "Figure",
  "number": "4.5.1",
  "title": "",
  "body": " (left) and (right) nullclines as well as regions of the phase plane marked by whether the state variable is increasing or decreasing.    N-nullcline and regions where N is increasing or decreasing    R-nullcline and regions where R is increasing or decreasing    "
},
{
  "id": "fig-competition-all-nullcline-orderings",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#fig-competition-all-nullcline-orderings",
  "type": "Figure",
  "number": "4.5.2",
  "title": "",
  "body": " All possible orderings of the intercepts and resulting relative positions for the - and -nullclines for the competition model.    N-nullcline intercepts are both above the R-nullcline intercepts    nullclines cross with equilibria intercepts above the non-equilibria intercepts      nullclines cross with non-equilibria intercepts above the equilibria intercepts    R-nullcline intercepts are both above the N-nullcline intercepts    "
},
{
  "id": "subsec-diffeq-predator-prey-2",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#subsec-diffeq-predator-prey-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Lotka-Volterra Predator Prey model "
},
{
  "id": "fig-predator-prey-pp-nullclines",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#fig-predator-prey-pp-nullclines",
  "type": "Figure",
  "number": "4.5.3",
  "title": "",
  "body": " Nullclines in the phase space for the owl-mice predator-prey model.   N-nullcline intercepts are both above the R-nullcline intercepts   "
},
{
  "id": "subsec-diffeq-nullclines-code-3",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#subsec-diffeq-nullclines-code-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "implicit plots "
...
{
  "id": "subsec-diffeq-nullclines-code-6",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#subsec-diffeq-nullclines-code-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "contour graph "
...
{
  "id": "subsec-diffeq-nullclines-code-8",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#subsec-diffeq-nullclines-code-8",
  "type": "Example",
  "number": "4.5.4",
  "title": "",
  "body": "  For our first example, let us generate an estimate of the nullclines for the competition model. We will use state variables and for the system of differential equations with so that our carrying capacities are and , respectively. We will choose and , and and .    # Define our parameters my_p <- c(r1 = 0.2, r2 = 0.3, alpha = 0.75, beta = 1.1) # Define our rate functions f1 <- function(X, Y, params) { with(as.list(params), r1 * X * (1 - X - alpha * Y) ) } f2 <- function(X, Y, params) { with(as.list(params), r2 * Y * (1 - Y - beta * X) ) } # Make a vector field grid (relatively wide spacing) library(dplyr) # To get mutate (add extra columns) vf_grid <- expand.grid(X = seq(-0.5, 1.5, by = 0.2), Y = seq(-0.5, 1.5, by = 0.2)) |> mutate(dXdt = f1(X, Y, my_p), dYdt = f2(X, Y, my_p)) |> # rescale by speed mutate(speed = sqrt(dXdt^2 + dYdt^2)) |> mutate(Xdir = dXdt \/ speed, Ydir = dYdt \/ speed) # Make a contour grid (relatively close spacing) contour_grid <- expand.grid(X = seq(-0.5, 1.5, by = 0.02), Y = seq(-0.5, 1.5, by = 0.02)) |> mutate(dXdt = f1(X, Y, my_p), dYdt = f2(X, Y, my_p)) # Make a graph showing the vector field and nullclines library(ggplot2) library(ggquiver) nullcline_graph <- ggplot() + # Vector field geom_quiver(data = vf_grid, mapping = aes(x = X, y = Y, u = Xdir, v = Ydir), vecsize = 0.25) + # X nullcline geom_contour(data = contour_grid, mapping = aes(x = X, y = Y, z = dXdt, color = \"X\"), breaks = c(0), linetype = \"dashed\") + # Y nullcline geom_contour(data = contour_grid, mapping = aes(x = X, y = Y, z = dYdt, color = \"Y\"), breaks = c(0), linetype = \"dashed\") + # Legend update scale_color_discrete(breaks = c(\"X\",\"Y\"), name = \"nullcline\") show(nullcline_graph)     Nullclines and vector field associated with the competition model given in the   generated graph showing nullclines and vector field described in listing     "
},
{
  "id": "fig-competition-model-R-nullclines-poor",
  "level": "2",
  "url": "seq-diffeq-nullclines-predation-competition.html#fig-competition-model-R-nullclines-poor",
  "type": "Figure",
  "number": "4.5.7",
  "title": "",
  "body": " Recreated with by=0.06 , which results in a gap in the nullclines   remade graph showing nullclines and vector field described in listing with different step size   "
},
{
  "id": "seq-diffeq-equilibria-love-linearization",
  "level": "1",
  "url": "seq-diffeq-equilibria-love-linearization.html",
  "type": "Section",
  "number": "4.6",
  "title": "Equilibrium Analysis, Models of Love, and Linearization",
  "body": " Equilibrium Analysis, Models of Love, and Linearization   Equilibrium solutions are one of the most important concepts in understanding dynamical systems. We already know that we find equilibrium solutions by looking for the intersection of the nullclines. In the previous section, we saw that sometimes we can determine the stability of an equilibrium by looking at the direction of the vector field in all of the regions surrounding an equilibrium. Unfortunately, this is sometimes inconclusive. Even worse, it might be hard to visualize especially if there are more than two state variables.  We need a more definitive way to determine whether an equilibrium solution is stable or unstable. For our projection models involving sequences, we learned about the possibility of using the eigenvalues of the derivative matrix of the projection functions. We will be able to do the same thing for systems of differential equations. This time, however, we are looking at whether the eigenvalues of the matrix have positive or negative real part.    Linear Differential Equations and Romeo and Juliet  Suppose we have different state variables, and a system of differential equations for those variables, We say that the system is a linear system of differential equations if the rate functions are all just linear combinations of the state variables The coefficients define a square matrix with rows (for each equation) and columns (for the state variables), This system is often summarized as a vector differential equation where is the state vector containing all of state variables in order.  As our motivating example, we will consider a simple model for love developed by mathematician Steven Strogatz as a way to illustrate linear differential equations, which he called the Romeo-Juliet model of love. Let quantify Romeo's ardor toward Juliet, so that corresponds to Romeo feeling in love with Juliet and corresponds to Romeo feeling out of love with Juliet. We let quantify Juliet's ardor toward Romeo in a similar way.  In his play Romeo and Juliet , Shakespeare gives a dialogue between Romeo and his friend Benvolio:   Benvolio  What sadness lengthens Romeo's hours?    Romeo  Not having that, which, having, makes them short.    Benvolio  In love?    Romeo  Out.    Benvolio  Of love?    Romeo  Out of her favour, where I am in love.   In a very simplified interpretation of the relationship, Romeo initially has positive feelings of love toward Juliet but because he perceives that he is out of her favor, he is sad. From a modeling point of view, we will say that seeing Juliet's feelings be negative causes Romeo's feelings of love decrease .  Juliet, on the other hand, expresses coyness at Romeo in a different scene:   Juliet   If thou dost love, pronounce it faithfully:  Or if thou think'st I am too quickly won,  I'll frown and be perverse an say thee nay,  So thou wilt woo; but else, not for the world.    Again, we will take a simplified interpretation and say that Juliet's actual feelings move in the opposite direction of Romeo current feelings. If Romeo loves Juliet, Juliet's feelings for Romeo will decrease. If Romeo dislikes Juliet, Juliet tries to win him back by having her feelings increase.  The simplest model capturing these simplified (and nontextual) dynamics would be given by a system of differential equations This says that Romeo's feelings move in the direction of Juliets feelings by having a rate of change that is proportional to Juliet's current level of love. Meanwhile, Juliet's feelings have a rate of change that is proportional to the opposite of Romeo's current level of love.  The phase plane will be the plane. The nullcline is the line (horizontal axis). If , is moving to the right; if , is moving to the left. The nullcine is the line (vertical axis). If , is moving down; if , is moving up. There is only one equilibrium at , a state of mutual apathy.   Phase plane for the Romeo-Juliet model showing nullclines, equilibrium, and directions of motion.   phase plane with nullclines and directions of motion for Romeo and Juliet model    The preliminary phase plane analysis based on the nullclines suggest that the dynamics will have the state spiraling around the equilibrium. We can verify this numerically by calculating a solution and looking at the plot. Of course, we need to select parameter values, so we will choose and . There is no fixed sense of scale for this problem, so we will start with an initial value and , so that Romeo likes Juliet but Juliet starts out being indifferent.  The following listing will create a graph showing the phase plane with nullclines and the trajectory for this initial value problem as well as a graph showing the solutions as functions of time.    # Rate formulas f1 <- function(R, J, params) { with(as.list(params), a*J) } f2 <- function(R, J, params) { with(as.list(params), -b*R) } # Model parameters my_p <- c(a = 0.8, b = 0.2) # Rate function for ode solver rateFcn <- function(t, state, params) { with(as.list(state), { dRdt <- f1(R, J, params) dJdt <- f2(R, J, params) list( c(dRdt, dJdt) ) }) } # Find the numerical solution library(deSolve) R0 <- 1 J0 <- 0 times <- seq(0, 30, by=0.1) soln <- ode( c(R=R0, J=J0), times, rateFcn, my_p ) |> as.data.frame() # Create vector field data library(dplyr) vf_data <- expand.grid(R = seq(-1.5, 1.5, length = 21), J = seq(-1.5, 1.5, length = 21)) |> mutate( dRdt = f1(R, J, my_p), dJdt = f2(R, J, my_p)) |> mutate( speed = sqrt(dRdt^2 + dJdt^2) ) |> mutate( Rdir = dRdt\/speed, Jdir = dJdt\/speed) # Create the plot, we know nullclines exactly library(ggplot2) library(ggquiver) pp_graph <- ggplot() + geom_quiver(data = vf_data, mapping = aes(x = R, y = J, u = Rdir, v = Jdir), color = \"black\", vecsize = 0.5) + geom_hline(yintercept = 0, color = \"cyan\") + geom_vline(xintercept = 0, color = \"salmon\") + geom_path(data = soln, mapping = aes(x = R, y = J), linewidth = 2) + labs(x = \"R\", y = \"J\") + scale_color_discrete(breaks=c(\"R\",\"J\"), labels = c(\"R'=0\", \"J'=0\"), name = \"nullcline\") show(pp_graph) soln_graph <- ggplot(data = soln) + geom_line(mapping = aes(x = time, y = R, color=\"R\"), linewidth = 2) + geom_line(mapping = aes(x = time, y = J, color=\"J\"), linewidth = 2) + scale_color_discrete(breaks = c(\"R\",\"J\"), name = \"\") show(soln_graph)     Phase plane showing nullclines and the trajectory of the solution as well as the solution and as functions of time.    phase plane showing nullclines and trajectory of a solution    phase plane showing nullclines and trajectory of a solution     If you think those graphs look like sine and cosine curves, you are correct. is exactly for some angular frequency . The love of Romeo and Juliet, if not interrupted, would be a never-ending cycle of ups and downs. But what is the angular frequency?  If we calculate we get , which by our differential equation is supposed to equal . Although we don't know quite yet, this allows us to solve for .  Another derivative gives us , which is also supposed to equal . Since we know , we must have or . Since our demo had and , we get .  Now I want to make a connection to matrices and eigenvalues. Our system of differential equations is a linear system because the rate formulas are linear combinations of the state variables and . The matrix associated with this system contains the coefficients of the variables, . What are the eigenvalues of this matrix?    A <- matrix( c(0, -0.2, 0.8, 0), nrow=2, ncol=2, byrow=FALSE) eig_sys <- eigen(A) print(eig_sys$values)     > print(eig_sys$values) [1] 0+0.4i 0-0.4i    The eigenvalues are complex numbers, and . The imaginary coefficient, the number multiplying , is exactly the same as our angular frequency.    Linear Systems of Differential Equations and Eigenvalues  The observation that the eigenvalues of the matrix were related to the solutions of the system of differential equations is not an accident. It is not our goal to learn the theory, but I will summarize the major results.    Suppose a linear system of differential equations for state variables is written in matrix form with a square matrix . With some rare exceptions, the matrix will have eigenvalues and corresponding eigenvectors .  Each eigenvalue eigenvector pair provides one component of a solution.   If is a real number, then there is a solution corresponding to either exponential growth or decay in the direction of     If is a pure imaginary number, , then its corresponding eigenvector will have a nonzero real and imaginary vector components and there will be two component solutions of the form and where are in the plane of the real and imaginary components of the eigenvector.  The conjugate value will also be an eigenvalue and its corresponding eigenvectors will similarly be conjugates. The conjugate does not introduce any new solutions.    If is a complex number with real and imaginary parts, , then its corresponding eigenvector will also have a nonzero real and imaginary vector components. There will be two component solutions of the form and where are in the plane of the real and imaginary components of the eigenvector.     Growth or decay ultimately comes down to the real part of the eigenvalues. For each eigenvalue with a positive real part, there is a eigenvector-based direction for which solutions see exponential growth. For each eigenvalue with a negative real part, there is a eigenvector-based direction for which solutions see exponential decay. Eigenvalues that are complex conjugate pairs together correspond to two directions in which there is oscillation, and whether the oscillation grows or decays is based on the real part of the eigenvalue.    The take-home message for this theorem is that when we have our matrix for the linear system of differential equations, we look at all of the eigenvalues and keep track of whether the real part is positive or negative. This will give us an understanding of whether solutions are growing or decaying. The imaginary parts will give us an understanding of whether there are any oscillations.    A variant of the Romeo and Juliet model is to consider the possibility that both Romeo and Juliet respond positively to the others' current emotions. That is, if Romeo loves Juliet, Juliets feelings toward Romeo increase, regardless of their current value. If Romeo dislikes Juliet, then her feelings will decrease. The mutually responsive model of love is given by   The matrix associated with this system is . Using particular values and , we find two real eigenvalues and . R reports the first eigenvector as , which will be the direction of the exponential growth component . The second eigenvector gives the direction of the exponential decay component .  The equilibrium of no passion in this case is unstable because solutions have components that move away from the equilibrium with exponential growth.      Linearization Around Equilibria  We are now ready to give the result for local stability analysis of equilibria in systems of differential equations. Suppose we have a system of differential equations linear or nonlinear  And suppose we have found an equilibrium solution at which all of the rate functions equal zero. Then near the equilibrium, the displacement of our solution from the equilibrium will approximately follow the linearized system of differential equations formed by the partial derivatives.  Let be the matrix of partial derivatives, and we evaluate the matrix at the equilibrium to get a specific matrix . Compute the eigenvalues of this matrix.  If all of the eigenvalues have negative real parts, then the equilibrium is locally stable.  If one or more of the eigenvalues have positive real parts, then the equilibrium is locally unstable.  If one or more of the eigenvalues has a real part that is 0 and none have positive real part, then this test is inconclusive.    The following Sage script can be modified to define the state variables and rate functions for your model and it will attempt to find algebraic solutions for equilibria. For each equilibrium solution it finds, it will report back the eigenvalues so that you can interpret the results. This is essentially the same analysis as what we did earlier for projection functions.  The provided example is for the nondimensionalized producer consumer model, which has two free parameters and . The code initial analyzes the system for and .   The matrix of partial derivatives in terms of the parameters and state variables will be .  Using the parameters and , we find the following results:   Equilibrium:  Eigenvalues: ,  Both eigenvalues are real, and one of them is positive. So this equilibrium is unstable    Equilibrium:  Eigenvalues: ,  Both eigenvalues are real and negative. So this equilibrium is stable    Equilibrium:  Eigenvalues: ,  Both eigenvalues are real, and one of them is positive. So this equilibrium (which is not biologically meaningful since one population is negative) is unstable   With these parameter values, the system is only at a stable equilibrium when the algae is at its natural carrying capacity and there are no zooplankton.  Changing the parameters to and , we repeat the analysis:   Equilibrium:  Eigenvalues: ,  Both eigenvalues are real, and one of them is positive. So this equilibrium is unstable    Equilibrium:  Eigenvalues: ,  Both eigenvalues are real, but one is still positive. This equilibrium is also unstable    Equilibrium:  Eigenvalues: ,  Both eigenvalues are real and negative. This equilibrium is stable   With these new parameter values, the system is stable when there are both algae and zooplankton present at equilibrium. If the population started with only algae at equilibrium , then introducing just a few zooplankton will result in the zooplankton growing to reach a positive equilibrium value.   "
},
{
  "id": "subsec-diffeq-equil-romeo-juliet-2",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#subsec-diffeq-equil-romeo-juliet-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "linear system of differential equations "
},
{
  "id": "fig-romeo-juliet-responsive-fickle-nullclines",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#fig-romeo-juliet-responsive-fickle-nullclines",
  "type": "Figure",
  "number": "4.6.1",
  "title": "",
  "body": " Phase plane for the Romeo-Juliet model showing nullclines, equilibrium, and directions of motion.   phase plane with nullclines and directions of motion for Romeo and Juliet model   "
},
{
  "id": "subsec-diffeq-equil-romeo-juliet-11",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#subsec-diffeq-equil-romeo-juliet-11",
  "type": "Listing",
  "number": "4.6.2",
  "title": "",
  "body": "  # Rate formulas f1 <- function(R, J, params) { with(as.list(params), a*J) } f2 <- function(R, J, params) { with(as.list(params), -b*R) } # Model parameters my_p <- c(a = 0.8, b = 0.2) # Rate function for ode solver rateFcn <- function(t, state, params) { with(as.list(state), { dRdt <- f1(R, J, params) dJdt <- f2(R, J, params) list( c(dRdt, dJdt) ) }) } # Find the numerical solution library(deSolve) R0 <- 1 J0 <- 0 times <- seq(0, 30, by=0.1) soln <- ode( c(R=R0, J=J0), times, rateFcn, my_p ) |> as.data.frame() # Create vector field data library(dplyr) vf_data <- expand.grid(R = seq(-1.5, 1.5, length = 21), J = seq(-1.5, 1.5, length = 21)) |> mutate( dRdt = f1(R, J, my_p), dJdt = f2(R, J, my_p)) |> mutate( speed = sqrt(dRdt^2 + dJdt^2) ) |> mutate( Rdir = dRdt\/speed, Jdir = dJdt\/speed) # Create the plot, we know nullclines exactly library(ggplot2) library(ggquiver) pp_graph <- ggplot() + geom_quiver(data = vf_data, mapping = aes(x = R, y = J, u = Rdir, v = Jdir), color = \"black\", vecsize = 0.5) + geom_hline(yintercept = 0, color = \"cyan\") + geom_vline(xintercept = 0, color = \"salmon\") + geom_path(data = soln, mapping = aes(x = R, y = J), linewidth = 2) + labs(x = \"R\", y = \"J\") + scale_color_discrete(breaks=c(\"R\",\"J\"), labels = c(\"R'=0\", \"J'=0\"), name = \"nullcline\") show(pp_graph) soln_graph <- ggplot(data = soln) + geom_line(mapping = aes(x = time, y = R, color=\"R\"), linewidth = 2) + geom_line(mapping = aes(x = time, y = J, color=\"J\"), linewidth = 2) + scale_color_discrete(breaks = c(\"R\",\"J\"), name = \"\") show(soln_graph)   "
},
{
  "id": "fig-romeo-juliet-responsive-fickle-graphs",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#fig-romeo-juliet-responsive-fickle-graphs",
  "type": "Figure",
  "number": "4.6.3",
  "title": "",
  "body": " Phase plane showing nullclines and the trajectory of the solution as well as the solution and as functions of time.    phase plane showing nullclines and trajectory of a solution    phase plane showing nullclines and trajectory of a solution    "
},
{
  "id": "subsec-diffeq-equil-romeo-juliet-16",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#subsec-diffeq-equil-romeo-juliet-16",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "linear system "
...
{
  "id": "subsec-diffeq-equil-romeo-juliet-17",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#subsec-diffeq-equil-romeo-juliet-17",
  "type": "Listing",
  "number": "4.6.4",
  "title": "",
  "body": "  A <- matrix( c(0, -0.2, 0.8, 0), nrow=2, ncol=2, byrow=FALSE) eig_sys <- eigen(A) print(eig_sys$values)   "
},
{
  "id": "thm-diffeq-linear-eigen-systems",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#thm-diffeq-linear-eigen-systems",
  "type": "Theorem",
  "number": "4.6.5",
  "title": "",
  "body": "  Suppose a linear system of differential equations for state variables is written in matrix form with a square matrix . With some rare exceptions, the matrix will have eigenvalues and corresponding eigenvectors .  Each eigenvalue eigenvector pair provides one component of a solution.   If is a real number, then there is a solution corresponding to either exponential growth or decay in the direction of     If is a pure imaginary number, , then its corresponding eigenvector will have a nonzero real and imaginary vector components and there will be two component solutions of the form and where are in the plane of the real and imaginary components of the eigenvector.  The conjugate value will also be an eigenvalue and its corresponding eigenvectors will similarly be conjugates. The conjugate does not introduce any new solutions.    If is a complex number with real and imaginary parts, , then its corresponding eigenvector will also have a nonzero real and imaginary vector components. There will be two component solutions of the form and where are in the plane of the real and imaginary components of the eigenvector.     Growth or decay ultimately comes down to the real part of the eigenvalues. For each eigenvalue with a positive real part, there is a eigenvector-based direction for which solutions see exponential growth. For each eigenvalue with a negative real part, there is a eigenvector-based direction for which solutions see exponential decay. Eigenvalues that are complex conjugate pairs together correspond to two directions in which there is oscillation, and whether the oscillation grows or decays is based on the real part of the eigenvalue.   "
},
{
  "id": "subsec-diffeq-eigenvalues-as-rates-5",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#subsec-diffeq-eigenvalues-as-rates-5",
  "type": "Example",
  "number": "4.6.6",
  "title": "",
  "body": "  A variant of the Romeo and Juliet model is to consider the possibility that both Romeo and Juliet respond positively to the others' current emotions. That is, if Romeo loves Juliet, Juliets feelings toward Romeo increase, regardless of their current value. If Romeo dislikes Juliet, then her feelings will decrease. The mutually responsive model of love is given by   The matrix associated with this system is . Using particular values and , we find two real eigenvalues and . R reports the first eigenvector as , which will be the direction of the exponential growth component . The second eigenvector gives the direction of the exponential decay component .  The equilibrium of no passion in this case is unstable because solutions have components that move away from the equilibrium with exponential growth.   "
},
{
  "id": "subsec-diffeq-linearization-2",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#subsec-diffeq-linearization-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "linearized "
...
{
  "id": "subsec-diffeq-linearization-8",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#subsec-diffeq-linearization-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "unstable stable unstable "
},
{
  "id": "subsec-diffeq-linearization-9",
  "level": "2",
  "url": "seq-diffeq-equilibria-love-linearization.html#subsec-diffeq-linearization-9",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "unstable unstable stable "
},
{
  "id": "system-examples",
  "level": "1",
